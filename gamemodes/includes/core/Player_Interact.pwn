...
Player_InteractMenu(playerid, giveplayerid, menu = 0) {
	...
	switch(menu) {
		case 0: {
			...
			return ShowPlayerDialogEx(playerid, INTERACT_MAIN, DIALOG_STYLE_LIST, szTitle, szMiscArray, "Chon", "Dong");
		}
		case 1: {
			/*Ingredients\n\
			Pistol Ammo\t%d\n\
			...
			return ShowPlayerDialogEx(playerid, INTERACT_GIVE, DIALOG_STYLE_TABLIST_HEADERS, szTitle, szMiscArray, "Chon", "Thoat");
		}
		case 2: {
			new itemid = GetPVarInt(playerid, "Interact_GiveItem");

			if(GetPVarType(playerid, "Interact_Drug")) {
				format(szMiscArray, sizeof(szMiscArray), "Ban muon cho %s bao nhieu phan trong so %s", GetPlayerNameEx(giveplayerid), Drugs[GetPVarInt(playerid, "Interact_Drug")]);
			}
			...
			return ShowPlayerDialogEx(playerid, INTERACT_AMOUNT, DIALOG_STYLE_INPUT, szTitle, szMiscArray, "Chon", "Thoat");
		}
		case 3: {
			...
			return ShowPlayerDialogEx(playerid, INTERACT_WEAPON, DIALOG_STYLE_LIST, szTitle, szMiscArray, "Give", "Thoat");
		}
		case 4: {
			new itemid = GetPVarInt(playerid, "Interact_GiveItem");
			...
			if(GetPVarType(playerid, "Interact_SellGun")) {
				...
				format(szMiscArray, sizeof(szMiscArray), "Ban muon ban %s cho %s voi gia bao nhieu?", ReturnWeaponName(weaponid), GetPlayerNameEx(giveplayerid));
			}
			if(GetPVarType(playerid, "Interact_Drug")) {

				new drugid = GetPVarInt(playerid, "Interact_Drug");

				format(szMiscArray, sizeof(szMiscArray), "Ban muon ban %dg %s cho %s voi gia bao nhieu?", amount, Drugs[drugid], GetPlayerNameEx(giveplayerid));
			}
			else format(szMiscArray, sizeof(szMiscArray), "Ban muon ban %d %s cho %s voi gia bao nhieu?", amount, Item_Getname(itemid), GetPlayerNameEx(giveplayerid));

			return ShowPlayerDialogEx(playerid, INTERACT_SELL, DIALOG_STYLE_INPUT, szTitle, szMiscArray, "Ban", "Thoat");
		}
		...
		case 5: {
			...
			if(GetPVarType(playerid, "Interact_SellGun")) {
				...
				format(szMiscArray, sizeof(szMiscArray), "[Tuong Tac]: Ban da de nghi %s mua mot %s voi gia $%s", GetPlayerNameEx(giveplayerid), Item_Getname(itemid), number_format(offerprice));
				SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
				format(szMiscArray, sizeof(szMiscArray), "%s da de nghi ban mua mot %s voi gia $%s", GetPlayerNameEx(playerid), ReturnWeaponName(weaponid), number_format(offerprice));
			}
			else if(GetPVarType(playerid, "Interact_Drug")) {
				new drugid = GetPVarInt(playerid, "Interact_Drug");

				format(szMiscArray, sizeof(szMiscArray), "[Tuong Tac]: Ban da de nghi %s mua %dg %s {FFFFFF}voi gia $%s", GetPlayerNameEx(giveplayerid), amount, Drugs[drugid], number_format(offerprice));
				SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
				format(szMiscArray, sizeof(szMiscArray), "%s da de nghi ban mua %dg %s {FFFFFF}voi gia $%s", GetPlayerNameEx(playerid), amount, Drugs[drugid], number_format(offerprice));
			}
			else {
				format(szMiscArray, sizeof(szMiscArray), "[Tuong Tac]: Ban da de nghi %s mua %d %s voi gia $%s", GetPlayerNameEx(giveplayerid), amount, Item_Getname(itemid), number_format(offerprice));
				SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
		...
		case 6: {
			...
			return ShowPlayerDialogEx(playerid, INTERACT_DRUGS, DIALOG_STYLE_TABLIST_HEADERS, szTitle, szMiscArray, "Chon", "Thoat");
		}
	}
...
Player_GiveItem(playerid, giveplayerid, itemid, amount, saleprice = 0) {
	...
	if(amount <= 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the cho khong.");

	if(restarting) return SendClientMessageEx(playerid, COLOR_RED, "Dang khoi dong lai may chu giao dich bi vo hieu hoa.");

	if(saleprice != 0 && (GetPlayerCash(giveplayerid) < saleprice || saleprice < 0)) return SendClientMessage(giveplayerid, COLOR_GRAD2, "Ban khong co du tien");
	...
	switch(itemid) {
		...
		case ITEM_DRUG: {
			...
			new drugid = GetPVarInt(playerid, "Interact_Drug");
			if(PlayerInfo[playerid][pDrugs][drugid] < amount) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong co nhieu.");

			if(amount + PlayerInfo[giveplayerid][pDrugs][drugid] > Player_MaxCapacity(giveplayerid, itemid)) {
				format(szMiscArray, sizeof(szMiscArray), "Nguoi do chi co the giu them %d vat pham do.", Player_LeftCapacity(giveplayerid, itemid));
		...
		case ITEM_MATS: {
			...
			if(PlayerInfo[playerid][pMats] < amount) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong co nhieu vat lieu.");
		...
		case ITEM_FIREWORK: {
			...
			if(PlayerInfo[playerid][pFirework] < amount) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong co nhieu phao hoa.");
		...
		case ITEM_SYRINGES: {
			...
			if(PlayerInfo[playerid][pSyringes] < amount) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong co nhieu ong tiem.");
		...
		case ITEM_SPRUNKDRINK: {
			...
			if(PlayerInfo[playerid][pSprunk] < amount) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban co nhieu nuoc ngot.");
		...
		case ITEM_PBTOKENS: {
			...
			if(PlayerInfo[playerid][pPaintTokens] < amount) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong co nhieu PB tokens.");
	...
	if(saleprice == 0) {
		...
		format(szMiscArray, sizeof(szMiscArray), "%s da cho ban %d %s", GetPlayerNameEx(playerid), amount, Item_Getname(itemid));
		...
		format(szMiscArray, sizeof(szMiscArray), "Ban da ban %s %d %s for $%s", GetPlayerNameEx(giveplayerid), amount, Item_Getname(itemid), number_format(saleprice));
		SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "%s da ban cho ban %d %s for $%s", GetPlayerNameEx(playerid), amount, Item_Getname(itemid), number_format(saleprice));
		SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
...
Interact_GivePlayerValidWeapon(playerid, giveplayerid, weaponid, saleprice = 0) {
	...
	if(saleprice != 0) {
		...
		format(szMiscArray, sizeof(szMiscArray), "Ban da ban %s a %s for $%s", GetPlayerNameEx(giveplayerid), ReturnWeaponName(weaponid), number_format(saleprice));
		SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "%s da ban cho ban mot %s $%s", GetPlayerNameEx(playerid), ReturnWeaponName(weaponid), number_format(saleprice));
		SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
		...
		format(szMiscArray, sizeof(szMiscArray), "ban mot %s voi gia $%s.", ReturnWeaponName(weaponid), number_format(saleprice));
		DBLog(playerid, giveplayerid, "ItemTransfer", szMiscArray);
	}
	else {
		format(szMiscArray, sizeof(szMiscArray), "Ban da cho %s mot %s", GetPlayerNameEx(giveplayerid), ReturnWeaponName(weaponid));
		SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "%s da cho ban mot %s", GetPlayerNameEx(playerid), ReturnWeaponName(weaponid));
		SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
		...
		format(szMiscArray, sizeof(szMiscArray), "da cho mot %s.", ReturnWeaponName(weaponid));
		DBLog(playerid, giveplayerid, "ItemTransfer", szMiscArray);
	}

	format(szMiscArray, sizeof(szMiscArray), "%s da cho %s mot %s", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid), ReturnWeaponName(weaponid));
...
Interact_GivePlayerDrug(playerid, giveplayerid, drugid, saleprice = 0) {
	...
	if(amount + PlayerInfo[giveplayerid][pDrugs][drugid] > Player_MaxCapacity(giveplayerid, ITEM_DRUG)) {
		format(szMiscArray, sizeof(szMiscArray), "Nguoi do chi co the giu them %d vat pham do.", Player_LeftCapacity(giveplayerid, ITEM_DRUG));
		SendClientMessageEx(playerid, COLOR_GRAD2, szMiscArray);
		return Player_InteractMenu(playerid, giveplayerid);
	}
	...
	if(saleprice != 0) {
		...
		format(szMiscArray, sizeof(szMiscArray), "Ban da ban %s %dg %s voi gia $%s", GetPlayerNameEx(giveplayerid), amount, Drugs[drugid], number_format(saleprice));
		SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "%s da ban cho ban %dg %s voi gia $%s", GetPlayerNameEx(playerid), amount, Drugs[drugid], number_format(saleprice));
		SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
		...
		format(szMiscArray, sizeof(szMiscArray), "da ban %dg %s voi gia $%s.", amount, Drugs[drugid], number_format(saleprice));
		DBLog(playerid, giveplayerid, "ItemTransfer", szMiscArray);
	}
	else {
		format(szMiscArray, sizeof(szMiscArray), "Ban da cho %s %dg %s", GetPlayerNameEx(giveplayerid), amount, Drugs[drugid]);
		SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);

		format(szMiscArray, sizeof(szMiscArray), "%s da cho ban %dg %s", GetPlayerNameEx(playerid), amount, Drugs[drugid]);
		SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
		...
		format(szMiscArray, sizeof(szMiscArray), "da dua %dg %s.", amount, Drugs[drugid]);
		DBLog(playerid, giveplayerid, "ItemTransfer", szMiscArray);
	}

	format(szMiscArray, sizeof(szMiscArray), "%s da dua %s mot so %s", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid), Drugs[drugid]);
	SetPlayerChatBubble(playerid, szMiscArray, COLOR_PURPLE, 10, 5000);
	return 1;
}
...
Interact_PayPlayer(playerid, giveplayerid, amount = -1) {
	...
	if(amount == -1) {
		format(szMiscArray, sizeof(szMiscArray), "Vui long nhap so tien de cung cap cho %s.", GetPlayerNameEx(giveplayerid));
		return ShowPlayerDialogEx(playerid, PAY_PLAYER, DIALOG_STYLE_INPUT, "Chuyen Tien Cho Nguoi Choi", szMiscArray, "Chuyen", "");
	}
	else {

		if(!(0 < amount <= 250000)) return SendClientMessageEx(playerid, COLOR_WHITE, "So tien toi da la $250.000 va toi thieu la $1");
		if (!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi choi khong o gan ban.");

		if(PlayerInfo[playerid][pCash] < amount) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co du tien.");

		format(szMiscArray, sizeof(szMiscArray), "Ban da dua %s $%s", GetPlayerNameEx(giveplayerid), number_format(amount));
		SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "Ban da duoc dua $%s boi %s", number_format(amount), GetPlayerNameEx(playerid));
		SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
		...
		Log("logs/pay.log", szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "da duoc dua $%s", number_format(amount));
...
Interact_CuffPlayer(playerid, giveplayerid) {
	...
	if(PlayerInfo[playerid][pHasCuff] < 1) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong co bat ky cai cong nao trong nguoi!");

	new Float:health, Float:armor;
	if(!IsPlayerConnected(giveplayerid))
	if (!ProxDetectorS(8.0, playerid, giveplayerid))
	if(GetPVarInt(giveplayerid, "Injured") == 1) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong the cong ai do trong tinh trang bi thuong.");
	if(PlayerCuffed[giveplayerid] != 1 && GetPlayerSpecialAction(giveplayerid) != SPECIAL_ACTION_HANDSUP) return SendClientMessage(playerid, COLOR_WHITE, "Nguoi choi do khong bi han che");

	if(PlayerInfo[giveplayerid][pConnectHours] < 32) SendClientMessageEx(giveplayerid, COLOR_WHITE, "Neu ban dang xuat ngay bay gio, ban se tu dong bi bo tu trong 2+ gio!");
	...
	format(szMiscArray, sizeof(szMiscArray), "* Ban da cong tay %s, cho den khi coi cong.", GetPlayerNameEx(giveplayerid));
	SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "* %s cong tay %s, siet chat chiec cong.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
	...
	if(GetPVarType(giveplayerid, "IsTackled")) {
	    format(szMiscArray, sizeof(szMiscArray), "* %s lay cai cong khoi that lung va co gang cong %s.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
		ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
...
Interact_UncuffPlayer(playerid, giveplayerid) {
	...
	if (!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi choi do khong gan ban.");
...
Interact_TakeDrugs(playerid, giveplayerid) {
	...
	format(szMiscArray, sizeof(szMiscArray), "** %s da bi tich thu %s's drugs.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
	ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
	return 1;
}
...
Interact_ShowLicenses(playerid, giveplayerid) {
	...
	if(PlayerInfo[playerid][pCarLic] == 0) { text1 = "Chua Co"; }
	else { text1 = date(PlayerInfo[playerid][pCarLic], 1); }
	if(PlayerInfo[playerid][pFlyLic]) { text4 = "Da Co"; } else { text4 = "Chua Co"; }
	if(PlayerInfo[playerid][pBoatLic]) { text2 = "Da Co"; } else { text2 = "Chua Co"; }
	if(PlayerInfo[playerid][pTaxiLicense]) { text3 = "Da Co"; } else { text3 = "Chua Co"; }
	if(PlayerInfo[playerid][pGunLic] == 0) {text5 = "Chua Co"; }
	...
	switch(PlayerInfo[playerid][pNation]) {
		...
		case 2: SendClientMessageEx(giveplayerid, COLOR_TR, "** Khong Co Quoc Tich **");
	}
	format(szMiscArray, sizeof(szMiscArray), "Liet ke giay phep cua %s's ...", GetPlayerNameEx(playerid));
	SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "Ngay Sinh: %s", PlayerInfo[playerid][pBirthDate]);
	SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "** Bang Lai Xe: %s.", text1);
	SendClientMessageEx(giveplayerid, COLOR_GREY, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "** Bang Lai May Bay: %s.", text4);
	SendClientMessageEx(giveplayerid, COLOR_GREY, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "** Bang Lai Thuyen: %s.", text2);
	SendClientMessageEx(giveplayerid, COLOR_GREY, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "** Bang Lai Taxi: %s.", text3);
	SendClientMessageEx(giveplayerid, COLOR_GREY, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "** Giay Phep SA Firearm: %s.", text5);
	SendClientMessageEx(giveplayerid, COLOR_GREY, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "* %s da cho ban xem giay phep cua ho.", GetPlayerNameEx(playerid));
	SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "* Ban da hien thi giay phep cua minh cho %s.", GetPlayerNameEx(giveplayerid));
	SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szMiscArray);
	return 1;
}
...
Interact_FriskPlayer(playerid, giveplayerid) {
	...
	if (!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong nang trong pham vi cua nguoi choi do");
	...
	format(szMiscArray, sizeof(szMiscArray), "Liet Ke Tui Do Cua %s.", GetPlayerNameEx(giveplayerid));
	SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
	SendClientMessageEx(playerid, COLOR_WHITE, "** Vat Pham **");
	if(PlayerInfo[giveplayerid][pMats] > 0)
	{
		format(szMiscArray, sizeof(szMiscArray), "(Pocket) %d Vat Lieu.", PlayerInfo[giveplayerid][pMats]);
    ...
    if(packages > 0)
	{
		format(szMiscArray, sizeof(szMiscArray), "(Pocket) %d Goi Vat Lieu.", packages);
		SendClientMessageEx(playerid, COLOR_GREY, szMiscArray);
	}
	if(crates > 0)
	{
		format(szMiscArray, sizeof(szMiscArray), "(Pocket) %d Thung Thuoc.", crates);
		SendClientMessageEx(playerid, COLOR_GREY, szMiscArray);
	}
	...
	{
		format(szMiscArray, sizeof(szMiscArray), "(Pocket) %d Ca.", PlayerInfo[giveplayerid][pFishes]);
		SendClientMessageEx(playerid, COLOR_GREY, szMiscArray);
	}
...
Interact_DragPlayer(playerid, giveplayerid) {
	...
	if(GetPVarInt(giveplayerid, "PlayerCuffed") != 2) return SendClientMessageEx(playerid, COLOR_WHITE, " Nguoi duoc chi dinh khong bi cong !");

	if(IsPlayerInAnyVehicle(playerid)) return SendClientMessageEx(playerid, COLOR_WHITE, " Ban phai ra khoi xe de su dung lenh nay.");
	if(GetPVarInt(giveplayerid, "BeingDragged") == 1) return SendClientMessageEx(playerid, COLOR_WHITE, " Nguoi choi do da bi dan di. ");

    new
    	Float:TempPos[3];

	GetPlayerPos(giveplayerid, TempPos[0], TempPos[1], TempPos[2]);
	if(!IsPlayerInRangeOfPoint(playerid, 5.0, TempPos[0], TempPos[1], TempPos[2])) return SendClientMessageEx(playerid, COLOR_GRAD2, " Nguoi do khong o gan ban.");

	format(szMiscArray, sizeof(szMiscArray), "* %s hien dang keo ban.", GetPlayerNameEx(playerid));
	SendClientMessageEx(giveplayerid, COLOR_WHITE, szMiscArray);

	format(szMiscArray, sizeof(szMiscArray), "* Ban dang keo %s, ban co the di chuyen chung ngay bay gio.", GetPlayerNameEx(giveplayerid));
	SendClientMessageEx(playerid, COLOR_WHITE, szMiscArray);

	format(szMiscArray, sizeof(szMiscArray), "* %s nam lay %s va bat dau dan di.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
	ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);

	SendClientMessageEx(playerid, COLOR_WHITE, "Bay gio ban dang keo toi pham, hay nhan vao '{AA3333}FIRE{FFFFFF}' de dung lai.");
...
Interact_DetainPlayer(playerid, giveplayerid, seatid = -1) {
	...
	if(seatid == -1) {
		format(szMiscArray, sizeof(szMiscArray), "Vui long nhap cho ngoi (1-3) de giam %s vao xe.", GetPlayerNameEx(giveplayerid));
		return ShowPlayerDialogEx(playerid, DETAIN_SEAT, DIALOG_STYLE_INPUT, "Giam Nguoi Choi", szMiscArray, "Giam", "");
	}
	else {
		if(IsPlayerInAnyVehicle(giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi do dang o trong o to - hay dua ho ra truoc.");
		if (!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong nam trong pham vi cua nguoi choi do!");
		if(PlayerCuffed[giveplayerid] != 2) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi choi do khong bi cong");

		new carid = gLastCar[playerid];
		if(!IsSeatAvailable(carid, seatid)) {
			SendClientMessageEx(playerid, COLOR_GREY, "Ghe do da duoc lay!");
			return Interact_DetainPlayer(playerid, giveplayerid);
		}
		...
		format(szMiscArray, sizeof(szMiscArray), "* Ban da bi dua len xe boi %s .", GetPlayerNameEx(playerid));
		SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "* Ban da dua %s len xe.", GetPlayerNameEx(giveplayerid));
		SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "* %s dua %s len xe.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
		ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
...
Interact_GiveTicket(playerid, giveplayerid, reason[], amount = -1) {
	if(amount == -1) {
		format(szMiscArray, sizeof(szMiscArray), "Vui long nhap so tien de phat %s", GetPlayerNameEx(giveplayerid));
		return ShowPlayerDialogEx(playerid, GIVE_TICKET, DIALOG_STYLE_INPUT, "Ticket Player", szMiscArray, "Tiep tuc", "");
	}
	if(isnull(reason)) {
		format(szMiscArray, sizeof(szMiscArray), "Vui long nhap ly do chinh dang cho %s", GetPlayerNameEx(giveplayerid));
		return ShowPlayerDialogEx(playerid, TICKET_REASON, DIALOG_STYLE_INPUT, "Ticket Player", szMiscArray, "Xong", "");
	}
	else {
		if(!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong nam trong pham vi cua nguoi choi do");

		format(szMiscArray, sizeof(szMiscArray), "* Ban da dua %s mot ve phat voi so tien $%d, ly do: %s", GetPlayerNameEx(giveplayerid), amount, reason);
		SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "* Si Quan %s da dua ban mot ve phat voi so tien $%d, ly do: %s", GetPlayerNameEx(playerid), amount, reason);
		SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "* Si Quan %s viet mot phat va dua no cho %s.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
		ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
		SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, "* Go /chapnhan ticket, de chap nhan ve phat.");
...
Interact_LoadPatient(playerid, giveplayerid, seatid = -1) {
	...
	if(seatid == -1) {
		format(szMiscArray, sizeof(szMiscArray), "Vui long nhap ID cho ngoi (1-3) de dua %s len xe.", GetPlayerNameEx(giveplayerid));
		return ShowPlayerDialogEx(playerid, PATIENT_SEAT, DIALOG_STYLE_INPUT, "Load Patient", szMiscArray, "Load", "");
	}
	else {
		if(GetPVarInt(giveplayerid, "Injured") != 1) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi do khong bi thuong - Ban khong the dua ho len xe.");
        if(IsPlayerInAnyVehicle(giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Benh nhan do dang o trong xe - Ban khong the dua len duoc nua.");
        if (!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong o gan nguoi choi do.");

        new carid = gLastCar[playerid];
        if(!IsAnAmbulance(carid)) return SendClientMessageEx(playerid, COLOR_GREY, "Phuong tien cuoi cung cua ban khong phai la xe Cuu Thuong.");
        if(IsVehicleOccupied(carid, seatid)) return SendClientMessageEx(playerid, COLOR_GREY, "Cho ngoi do da co nguoi.");
		if(!IsPlayerInRangeOfVehicle(giveplayerid, carid, 10.0) || !IsPlayerInRangeOfVehicle(playerid, carid, 10.0)) return SendClientMessageEx(playerid, COLOR_GREY, "Ca ban va benh nhan cua ban phai o gan xe Cuu Thuong.");

		format(szMiscArray, sizeof(szMiscArray), "* Ban da duoc dua len xe boi Nhan Vien Y te %s.", GetPlayerNameEx(playerid));
		SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "* Ban da dua benh nhan %s len xe.", GetPlayerNameEx(giveplayerid));
		SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szMiscArray);
		format(szMiscArray, sizeof(szMiscArray), "* %s da dua benh nhan %s vao trong %s.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid), GetVehicleName(carid));
		ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
...
Interact_Triage(playerid, giveplayerid) {
	if(PlayerInfo[playerid][pTriageTime] != 0)	return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi 2 phut de su dung lenh nay.");
	if(!IsPlayerConnected(giveplayerid)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nguoi choi khong duoc ket noi.");
    if(!ProxDetectorS(5.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nguoi choi do khong o gan ban!");

    new Float: health;
    GetHealth(giveplayerid, health);
    if(health >= 85) SetHealth(giveplayerid, 100);
	else SetHealth(giveplayerid, health+15.0);
    format(szMiscArray, sizeof(szMiscArray), "* %s da cho %s 15 Suc Khoe.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
    ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
...
Interact_Heal(playerid, giveplayerid, healprice = -1) {
	...
	if(healprice == -1) {
		format(szMiscArray, sizeof(szMiscArray), "Vui long nhap gia tien hoi phuc cho %d.", GetPlayerNameEx(giveplayerid));
		return ShowPlayerDialogEx(playerid, HEAL_PLAYER, DIALOG_STYLE_INPUT, "Heal Player", szMiscArray, "Hoi phuc", "");
	}
	else {
		new Float:X, Float:Y, Float:Z;
		GetPlayerPos(giveplayerid, X, Y, Z);

		if(!IsPlayerInRangeOfPoint(playerid, 10, X, Y, Z)) return SendClientMessageEx(playerid, TEAM_GREEN_COLOR,"Ban khong o gan ho!");

		new Float:tempheal;
		GetHealth(giveplayerid,tempheal);
		if(tempheal >= 100.0) return SendClientMessageEx(playerid, TEAM_GREEN_COLOR,"Nguoi do da hoan toan lanh lan.");

		format(szMiscArray, sizeof(szMiscArray), "Ban da chua lanh cho %s voi gia $%d.", GetPlayerNameEx(giveplayerid),healprice);
		...
		format(szMiscArray, sizeof(szMiscArray), "Ban da duoc hoi phuc den 100 mau voi $%d boi %s.",healprice, GetPlayerNameEx(playerid));
		SendClientMessageEx(giveplayerid, TEAM_GREEN_COLOR,szMiscArray);

		if(GetPVarType(giveplayerid, "STD")){
			DeletePVar(giveplayerid, "STD");
			SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, "* Ban khong con bi nhiem STD nua vi da co su giup do cua Bac Si.");
		}
	}
	return 1;
}
...
Interact_MovePatient(playerid, giveplayerid) {
	...
	if(GetPVarInt(giveplayerid,"Injured") != 1) return SendClientMessageEx(playerid, COLOR_WHITE, "Nguoi choi do khong bi thuong");

	if(IsPlayerInAnyVehicle(playerid)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the su dung lenh nay khi dang o tren xe.");
	if(PlayerInfo[giveplayerid][pJailTime] > 0) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the su dung lenh nay cho nhung nguoi choi bi bo tu.");
	if(GetPVarInt(giveplayerid, "OnStretcher") == 1) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nguoi do da nam tren Cang, ban khong the lam dieu nay ngay bay gio!");

	new Float:mX, Float:mY, Float:mZ;
	GetPlayerPos(giveplayerid, mX, mY, mZ);
	if(!IsPlayerInRangeOfPoint(playerid, 5.0, mX, mY, mZ)) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban phai o gan benh nhan de co the di chuyen ho!");

	SendClientMessageEx(playerid, COLOR_GRAD2, "Ban co 30 giay de di chuyen den mot vi tri khac hoac ban co the nhan nut '{AA3333}FIRE{BFC0C2}' de dung.");
	format(szMiscArray, sizeof(szMiscArray), "* Ban da duoc dat tren cang boi %s.", GetPlayerNameEx(playerid));

	SendClientMessageEx(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "* Ban da dat %s len tren cang, ban co the di chuyen ho ngay bay gio.", GetPlayerNameEx(giveplayerid));

	SendClientMessageEx(playerid, COLOR_LIGHTBLUE, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "* %s dat %s len tren cang, that chat day an toan.", GetPlayerNameEx(playerid), GetPlayerNameEx(giveplayerid));
...
Interact_Prescribe(playerid, stage = 0) {
	...
	switch(stage) {
		...
		case 1: {
			...
			ShowPlayerDialogEx(playerid, DIALOG_STYLE_INPUT, INTERACT_PRESCRIBE1, "Grams | Drug Prescription", "Ban muon ke bao nhieu?", "Ke don", "Thoat");
		}
	}
	return 1;
}
...
Interact_ProcessPrescription(playerid) {
	...
	format(szMiscArray, sizeof(szMiscArray), "Ban da ke cho %s %d vien %s.", GetPlayerNameEx(giveplayerid), iAmount, szMiscArray);
	SendClientMessage(playerid, COLOR_GRAD2, szMiscArray);
	format(szMiscArray, sizeof(szMiscArray), "%s da ke cho ban %d vien %s.", GetPlayerNameEx(playerid), iAmount, szMiscArray);
...
CMD:tuongtac(playerid, params[]) {
	...
	if(isnull(params)) return SendClientMessageEx(playerid, COLOR_GREY, "SU DUNG: /interact [playerid]");
	...
	if(giveplayerid == INVALID_PLAYER_ID) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi choi khong truc tuyen.");
	if(GetPVarInt(playerid, "Injured") == 1) return 1;
	if(!ProxDetectorS(8.0, playerid, giveplayerid)) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi choi khong o gan ban.");
	if(PlayerInfo[giveplayerid][pAdmin] >= 2 && !PlayerInfo[giveplayerid][pTogReports]) return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi choi khong o gan ban.");
