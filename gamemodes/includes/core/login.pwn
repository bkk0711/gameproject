...
stock ShowMainMenuDialog(playerid, frame)
{
	...
	switch(frame)
	{
		case 1:
		{
			...
			format(string, sizeof(string), "{FFFFFF}Chao mung ban den voi Los Santos Role Play, %s.\n\nTen ma ban su dung da duoc dang ky, vui long nhap mat khau de dang nhap:", GetPlayerNameEx(playerid));
		...
		case 2:
		{
			...
			format(string, sizeof(string), "{FFFFFF}Chao mung ban den voi Los Santos Role Play, %s.\n\n{FFFFFF}Ban co the {AA3333}dang ky {FFFFFF}mot tai khoan bang cach nhap mat khau vao duoi day:", GetPlayerNameEx(playerid));
			if(PassComplexCheck) strcat(string, "\n\n- Ban khong the chon mat khau duoi 8 hoac tren 64 ky tu\n\
			- Mat khau cua ban phai chua ket hop cac chu cai, so va cac nhan vat dac biet.\n\
		...
		{
			format(titlestring, sizeof(titlestring), "{3399FF}Dang nhap - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "{FFFFFF}Mat khau khong hop le!\n\nChao mung ban den voi Los Santos Role Play, %s.\n\nTen ma ban su dung da duoc dang ky, vui long nhap mat khau de dang nhap:", GetPlayerNameEx(playerid));
			ShowPlayerDialogEx(playerid,MAINMENU,DIALOG_STYLE_PASSWORD,titlestring,string,"Dang nhap","Thoat");
		}
		case 4:
		{
			format(titlestring, sizeof(titlestring), "{3399FF}Tai khoan bi khoa - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "{FFFFFF}Co so du lieu cua chung toi chi ra rang %s hien dang dang nhap, neu day la mot loi, vui long lien he voi Administrator de duoc giai quyet.", GetPlayerNameEx(playerid));
...
stock SafeLogin(playerid, type)
{
	switch(type)
	{
		...
		case 2: // No Account Exists
		{
			...
			{
				Dialog_Show(playerid, -1, DIALOG_STYLE_MSGBOX, "LOI: Ban bi duoi!", "Ban khong the tao tai khoan tren may chu nay!\n\nMay chu nay chi danh cho nhom Beta.\n\nNeu ban la nguoi thu nghiem Beta, hay truy cap LSR.VN va tao tai khoan.", "Dong", "");
...
stock InvalidNameCheck(playerid) {
	...
	while(i < sizeof(arrForbiddenNames)) if(strcmp(arrForbiddenNames[i++], GetPlayerNameExt(playerid), true) == 0) {
		...
		SendClientMessage(playerid, COLOR_RED, "Ban da bi duoi va dang nhap vi su dung ten nguoi dung bi cam.");
...
stock ShowMainMenuGUI(playerid)
{
	...
	//SetPVarInt(playerid, "LoginScreen", RandomEx(1,9));
...
public LoginCheck(playerid)
{
	...
	{
		new string[128];
		format(string, sizeof(string), "%s(%d) [%s] da het thoi gian dang nhap.", GetPlayerNameEx(playerid), GetPlayerSQLId(playerid), GetPlayerIpEx(playerid));
		Log("logs/security.log", string);
		SendClientMessage(playerid, COLOR_WHITE, "MAY CHU: Het thoi gian dang nhap - ban phai dang nhap trong vong 60 giay!");
...
stock ShowLoginDialogs(playerid, index)
{
	...
	switch(index)
	{
		case 0:
		{
			ShowPlayerDialogEx(playerid, DIALOG_CHANGEPASS2, DIALOG_STYLE_INPUT, "Yeu cau thay doi mat khau!", "Vui long nhap mat khau moi cho tai khoan cua ban.", "Thay doi", "Thoat" );
			if(PassComplexCheck) ShowPlayerDialogEx(playerid, DIALOG_CHANGEPASS2, DIALOG_STYLE_INPUT, "Yeu cau thay doi mat khau!", "Vui long nhap mat khau moi cho tai khoan cua ban.\n\n\
			- Ban khong the chon mat khau duoi 8 hoac tren 64 ky tu\n\
			- Mat khau cua ban phai chua ket hop cac chu cai, so va cac nhan vat dac biet.\n\
			- Nhan vat khong hop le: %", "Thay dio", "Thoat" );
		}
		case 1: 
		{
		...
		case 4: ShowPlayerDialogEx(playerid, PMOTDNOTICE, DIALOG_STYLE_MSGBOX, "Ghi chu", pMOTD, "Bo qua", "");
		case 5:
		{
			format(string, sizeof(string), "Ban da nhan duoc {FFD700}%s{A9C4E4} credits! Dung /shophelp de xem thong tin.", number_format(PlayerInfo[playerid][pReceivedCredits]));
			ShowPlayerDialogEx(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Credits Received!", string, "Dong", "");
