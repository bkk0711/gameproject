...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	switch(dialogid)
	{
		case DIALOG_PHONE_COLOR:
		{
			...
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Dung /phone de mo dien thoai cua ban. {BBBBBB}(Dung /pcl de lay lai con tro khi vo tinh bam huy tuong tac voi ban.)");
			return 1;
		}
		...
		case DIALOG_PHONE_CONTACTS:
		{
			...
			switch(listitem)
			{
				case 0: ShowPlayerDialogEx(playerid, DIALOG_PHONE_YPAGES, DIALOG_STYLE_TABLIST_HEADERS, "Trang Vang", "Ten\tSo dien thoai\n\
					--> Businesses\tMENU ->\n\
					...
					Cong ty sua chua xe Farva\t1-800-2232 ", "Select", "Cancel");
				...
				case 1: {
					...
					for(new i = 0; i < MAX_PAYPHONES; ++i) {
						...
						if(IsValidDynamicArea(arrPayPhoneData[i][pp_iAreaID])) {
							...
							format(szMiscArray, sizeof(szMiscArray), "%sKhu vuc: %d\tSo: %d\tKhu: %s\n", szMiscArray, GetPhoneAreaCode(i), arrPayPhoneData[i][pp_iNumber], szZone);
							ListItemTrackId[playerid][x] = arrPayPhoneData[i][pp_iNumber];
							x++;
						}
					}
					if(isnull(szMiscArray)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Khong co dien thoai tra tien.");
					ShowPlayerDialogEx(playerid, DIALOG_PHONE_PAYPHONES, DIALOG_STYLE_TABLIST_HEADERS, "Dien thoai tra phi", szMiscArray, "Goi", "Cancel");
				}
				case 2:	{
				...
				case 3: ShowPlayerDialogEx(playerid, DIALOG_PHONE_ADDCONTACT, DIALOG_STYLE_INPUT, "Dien thoai | Them lien he", "Vui long nhap ten cua lien he ban muon them.\nVi du: Hoang Vu", "them", "<<");
				case 4: {
		...
		case DIALOG_PHONE_YPAGES: {
			...
			switch(listitem) {
				...
				case 0: {
					...
					return ShowPlayerDialogEx(playerid, DIALOG_PHONE_BUSINESSES, DIALOG_STYLE_LIST, "Trang Vang | Businesses", "\
						24/7\n\
						Cua hang quan ao\n\
						Nha hang\n\
						Bars\n\
				...
				case 7: cmd_dichvu(playerid, "mechanic");
			}
		...
		case DIALOG_PHONE_BUSINESSES: {
			...
			if(!response) {
				...
				return ShowPlayerDialogEx(playerid, DIALOG_PHONE_YPAGES, DIALOG_STYLE_TABLIST_HEADERS, "Trang Vang", "Ten\tSo dien thoai\n\
					--> Businesses\tMENU ->\n\
					SA News\t1-800-1800\n\
					Government - City Hall/White House\t1-800-4444\n\
					BubbaGump Fish 'n Shrimps Shipment Service\t1-800-6666\n\
					...
					Cong ty sua xe Farva\t1-800-2232 ", "Select", "Cancel");
		...
		case DIALOG_PHONE_ADDCONTACT1: {
			...
			if(strlen(szMiscArray) > 16) {
				SendClientMessage(playerid, COLOR_YELLOW, "[DIEN THOAI] {DDDDDD} Ten qua dai. Vui long thu lai.");
				return ShowPlayerDialogEx(playerid, DIALOG_PHONE_ADDCONTACT1, DIALOG_STYLE_INPUT, "Them lien he | So", "Vui long nhap so lien lac ban muon them.", "Them", "Huy bo");
			}
		...
		case DIALOG_PHONE_SETTINGS:
		{
			...
			switch(listitem)
			{
				case 0:
				{
					ShowPlayerDialogEx(playerid, DIALOG_PHONE_RINGTONES, DIALOG_STYLE_LIST, "Dien thoai | Nhac chuong", "\
							Go Go Track!\n\
							Dual Dual!\n\
							Bee Bees\n\
							So many awards!\n\
		...
		case DIALOG_PHONE_RINGTONES:
		{
			...
			ShowPlayerDialogEx(playerid, DIALOG_PHONE_RINGTONES2, DIALOG_STYLE_MSGBOX, "Dien thoai | Nhac chuong | Xem truoc", "Nhan 'Select' de xac nhan.\nNhan 'Cancel' de huy bo.", "Select", "Cancel");
		}
...
Phone_Businesses(playerid, btype)
{
	...
	ShowPlayerDialogEx(playerid, DIALOG_PHONE_BUSINESSES2, DIALOG_STYLE_TABLIST, "San Andreas | Ban do | Businesses", szMiscArray, "Select", "Back");
	return 1;
}
...
CMD:tim(playerid, params[]) {
	return cmd_trace(playerid, params);
}
CMD:trace(playerid, params[]) {
	...
	if(Bit_State(arrPlayerBits[playerid], phone_bitTraceState)) {
		...
		return SendClientMessageEx(playerid, COLOR_GRAD1, "[Dien thoai]: Thoat khoi qua trinh theo doi GPS. Tat ca du lieu dia li da bi xoa.");
	}
	...
	if(gettime() < UsedFind[playerid]) { // || FindTimePoints[playerid] == 0) {
		return SendClientMessageEx(playerid, COLOR_GREY, "Ban da tim kiem ai do - doi mot chut.");
	}

	new iNumber;

	if(sscanf(params, "d", iNumber)) return SendClientMessage(playerid, COLOR_GRAD1, "Usage: /trace [so dien thoai cua nguoi choi]");

	if(iNumber == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh mot so khong hop le.");
	...
	foreach(new i : Player)	{
		...
		if(PlayerInfo[i][pPnumber] == iNumber) {
			...
			if(GetPlayerInterior(i) != 0)
				return SendClientMessageEx(playerid, COLOR_GREY, "Nguoi do o trong Interior.");

			if((PlayerInfo[i][pAdmin] >= 2 || PlayerInfo[i][pWatchdog] >= 2) && PlayerInfo[i][pTogReports] != 1)
				return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong the tim thay nguoi nay.");

			if(GetPVarInt(playerid, "_SwimmingActivity") >= 1)
				return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the tim nguoi dang boi.");
			...
			else return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the co mot dau vet tren nguoi nay.");
	...
	SendClientMessage(playerid, COLOR_GRAD1, "Khong co dien thoai hoat dong duoc phat hien voi so nay.");
	return 1;
}
...
public Phone_Trace(playerid, i)
{
	...
	if(x == 0) format(szMiscArray, sizeof(szMiscArray), "{DDDDDD}Chi dinh muc tieu: %d\nGPS Tim kiem dang tien hanh...\n______________________________________\n\n", PlayerInfo[i][pPnumber]);
	...
	if(IsPlayerInDynamicArea(x, arrRadioTowerData[x][rt_iAreaID]))
	{
		...
		switch(fDistance)
		{
			...
			case 1501 .. 2500: szStrength = "{FFFF00}Thuong";
			case 0 .. 1500: szStrength = "{00FF00}Tot";
		}
	}
	else szStrength = "{FF0000}TRONG";
	...
	if(!IsValidDynamicObject(arrRadioTowerData[x][rt_iObjectID]))
	{
		...
		format(szMiscArray, sizeof(szMiscArray), "%s\n\
			...
			{FFFFFF}%d's vi tri da duoc xac dinh: {FFFF00}%s.", szMiscArray, PlayerInfo[i][pPnumber], arrRadioTowerData[x][rt_szName]);
		ShowPlayerDialogEx(playerid, 0, DIALOG_STYLE_MSGBOX, "Tim so", szMiscArray, "-----", "");
		SendClientMessage(playerid, COLOR_YELLOW, "Dung /trace mot lan nua de dung truy tim so.");
		...
		format(szMiscArray, sizeof(szMiscArray), "%s da duoc nhin thay lan cuoi tai %s.", GetPlayerNameEx(i), szZone);
		...
		if(PlayerInfo[playerid][pDoubleEXP] > 0) {
			...
			format(szMiscArray, sizeof(szMiscArray), "Ban da dat duoc 2 dien ky nang tham tu thay vi 1. Ban co %d gio choi voi Double EXP token.", PlayerInfo[playerid][pDoubleEXP]);
		...
		switch(PlayerInfo[playerid][pDetSkill]) {
			case 50: SendClientMessageEx(playerid, COLOR_YELLOW, "* Ky nang Tham Tu cua ban bay gio la cap do 2, ban co the tim nhanh hon.");
			case 100: SendClientMessageEx(playerid, COLOR_YELLOW, "* Ky nang Tham Tu cua ban bay gio la cap do 3, ban co the tim nhanh hon.");
			case 200: SendClientMessageEx(playerid, COLOR_YELLOW, "* Ky nang Tham Tu cua ban bay gio la cap do 4, ban co the tim nhanh hon.");
			case 400: SendClientMessageEx(playerid, COLOR_YELLOW, "* Ky nang Tham Tu cua ban bay gio la cap do 5, ban co the tim nhanh hon.");
		}
	...
	{
		format(szMiscArray, sizeof(szMiscArray), "%sLay %d's du lieu di dong tu %s tower \t|\t Tin hieu: %s.{DDDDDD}\n\n", szMiscArray, PlayerInfo[playerid][pPnumber], arrRadioTowerData[x][rt_szName], szStrength);
		ShowPlayerDialogEx(playerid, 0, DIALOG_STYLE_MSGBOX, "Tim so", szMiscArray, "Tim...", "");
	}
...
public Phone_OnGetContacts(iPlayerID)
{
	...
	if(!iRows) return SendClientMessage(iPlayerID, COLOR_GRAD1, "Ban khong co bat ky lien he nao.");
	...
	if(GetPVarType(iPlayerID, PVAR_PHONEDELCONTACT)) return ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CONTACTLISTDEL, DIALOG_STYLE_TABLIST_HEADERS, "Dien thoai | Xoa lien he", szMiscArray, "Xoa", "<<");
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CONTACTLIST, DIALOG_STYLE_TABLIST_HEADERS, "Dien thoai | Danh sach lien he", szMiscArray, "Goi", "<<");
	return 1;
}
...
{
	if(mysql_errno()) return Phone_Contacts(iPlayerID), SendClientMessage(iPlayerID, COLOR_GRAD1, "Da xay ra loi. Vui long thu lai.");
	SendClientMessage(iPlayerID, COLOR_YELLOW, "[DIEN THOAI] {DDDDDD} Ban da them thanh cong lien he moi.");
...
{
	if(mysql_errno()) return SendClientMessage(iPlayerID, COLOR_GRAD1, "Da xay ra loi. Vui long thu lai.");
	SendClientMessage(iPlayerID, COLOR_YELLOW, "[DIEN THOAI] {DDDDDD} Ban da xoa thanh cong lien he.");
...
public Phone_CheckContacts(iPlayerID, name[])
{
	...
	if(iRows > MAX_CONTACTS) {
		SendClientMessage(iPlayerID, COLOR_GRAD1, "Ban khong the co nhieu lien he trong dien thoai.");
		DeletePVar(iPlayerID, "PHN_CONTACT");
	}
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_ADDCONTACT1, DIALOG_STYLE_INPUT, "Them lien he | So", "Vui long nhap so lien lac ban muon them.", "Them", "Huy bo");
	return 1;
}
...
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_DIAL, DIALOG_STYLE_INPUT, "Dien thoai | Quay so", "Vui long nhap so ban muon quay.", "Quay", "<<");
}

Phone_Contacts(iPlayerID)
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CONTACTS, DIALOG_STYLE_LIST, "Dien thoai | So dia chi", "\
		Trang Vang\n\
		Dien thoai tra phi\n\
		Danh sach lien he\n\
		Them lien he\n\
...
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_CAMERA, DIALOG_STYLE_LIST, "Dien thoai | May anh", "Back camera\nFront camera\nReset", "Lua chon", "<<");
}

Phone_Settings(iPlayerID)
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_SETTINGS, DIALOG_STYLE_LIST, "Dien thoai | Cai dat", "Nhac chuong\nHinh nen", "Lua chon", "<<");
}
...
Phone_Events(iPlayerID) {
	SendClientMessageEx(iPlayerID, COLOR_GRAD1, "Day se la mot tinh nang som!");
}
...
{
	ShowPlayerDialogEx(iPlayerID, DIALOG_PHONE_WALLPAPERS, DIALOG_STYLE_LIST, "Dien thoai | WallPapers", "\
		Standard\n\
		Vice City 1\n\
		Vice City 2\n\
		Cop Art 1\n\
...
Phone_PhoneColorMenu(playerid) {
	...
	ShowPlayerDialogEx(playerid, DIALOG_PHONE_COLOR, DIALOG_STYLE_LIST, "Dien thoai - Lua chon mau", "Black\nWhite\nRed\nPink\nOrange\nBrown\nYellow\nGreen\nBlue\nLight Baby Blue", "Chon", "");
}
...
CMD:dienthoai(playerid, params[]) {
	return cmd_phone(playerid, params);
}
...
Phone_Main(playerid) {
	if(GetPVarInt(playerid, "EventToken")) {
		...
		return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong the lam dieu nay bay gio!");
	}
	else if(GetPVarType(playerid, "FixVehicleTimer")) {
		return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban dang sua xe!");
	}
	if(PlayerCuffed[playerid] != 0 || GetPVarInt(playerid, "pBagged") >= 1 || PlayerInfo[playerid][pHospital] != 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the dung dien thoai bay gio.");
	...
	if(!Bit_State(arrPlayerBits[playerid], phone_bitState)) {
		...
		SendClientMessageEx(playerid, COLOR_WHITE, "[TRO GIUP DIEN THOAI]: Dung ~k~~GO_FORWARD~ de di len, ~k~~GO_BACK~ de di xuong va ~k~~PED_SPRINT~ lua chon.");
...
CMD:selfie(playerid, params[]) {
	...
	if(IsPlayerInAnyVehicle(playerid)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the dung trong xe.");
	if(GetPVarInt(playerid, "EventToken")) {
		return SendClientMessageEx(playerid, COLOR_GRAD1, "You can't use this while in an event.");
	}
		...
		return SendClientMessage(playerid, COLOR_GRAD2, "Ban khong the lam bay gio!");
	}
	else if(GetPVarType(playerid, "FixVehicleTimer")) {
		return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban dang sua xe!");
	}
	...
	if(!Bit_State(arrPlayerBits[playerid], phone_bitCamState))
	{
	    ...
	    SendClientMessage(playerid, COLOR_GRAD1, "{DDDDDD}Chon ung dung camera hoac dung /selfie de quay lai. Dung {FFFF00}/headmove {DDDDDD}de vo hieu hoa chuyen dong dau cua ban (nhin thang).");
	    Bit_On(arrPlayerBits[playerid], phone_bitCamState);
	}
	else
	{
...
Phone_InitTD(playerid) {
	...
	*/
	...
	phone_PTextDraw[playerid][6] = CreatePlayerTextDraw(playerid,571.000000, 376.000000, "huy bo");
	...
	phone_PTextDraw[playerid][11] = CreatePlayerTextDraw(playerid,518.000000, 376.000000, "lua chon");
	...
	phone_PTextDraw[playerid][12] = CreatePlayerTextDraw(playerid,510.000000, 266.000000, "Goi");
	...
	phone_PTextDraw[playerid][13] = CreatePlayerTextDraw(playerid,510.000000, 278.000000, "Lien he");
	...
	phone_PTextDraw[playerid][16] = CreatePlayerTextDraw(playerid,510.000000, 314.000000, "Am nhac");
	...
	phone_PTextDraw[playerid][18] = CreatePlayerTextDraw(playerid,510.000000, 338.000000, "Ban do Doogle");
	...
	phone_PTextDraw[playerid][19] = CreatePlayerTextDraw(playerid,510.000000, 350.000000, "May anh");
