...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	...
	if((newkeys & KEY_NO) && vehicleid != INVALID_VEHICLE_ID && GetPlayerState(playerid) == PLAYER_STATE_DRIVER) {
		...
		if(engine == VEHICLE_PARAMS_ON)
		{
			...
			format(szMiscArray, sizeof(szMiscArray), "{FF8000}** {C2A2DA}%s dut chia khoa trong o khoa va dong co dung lai.", GetPlayerNameEx(playerid));
		...
		else if((engine == VEHICLE_PARAMS_OFF || engine == VEHICLE_PARAMS_UNSET))
		{
			...
			format(szMiscArray, sizeof(szMiscArray), "{FF8000}** {C2A2DA}%s dut chia khoa trong o khoa va mo dong co.", GetPlayerNameEx(playerid));
			ProxDetector(30.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			SendClientMessageEx(playerid, COLOR_WHITE, "Dong co xe bat dau, vui long doi...");
		...
		if(v != -1) {
			if(IsABike(iVehID)) {
				if(PlayerVehicleInfo[i][v][pvLocked] == 1 && PlayerVehicleInfo[i][v][pvLock] > 0 && PlayerVehicleInfo[i][v][pvLocksLeft] > 0) {
					SendClientMessageEx(playerid, COLOR_WHITE, "Chiec xe dap nay hien dang bi khoa.");
					return 0;
				}
			}
		}
...
ShowVehicleMenu(playerid, vehicleid) {
	...
	format(szMiscArray, sizeof(szMiscArray), "Item\tTrang thai\n\
		That day an toan hoat mu bao hiem\t%s\n\
		Den\t%s\n\
		Bonnet\t%s\n\
		Boot\t%s",
