...
LoadATMPoints() {
	...
	ATMPoint[1] = CreateDynamicSphere(0, 0, 0, 3.0);
	...
	if((newkeys & KEY_YES) && IsPlayerInAnyDynamicArea(playerid))
	{
		...
		for(new i; i < sizeof(ATMPoint); ++i) {
			...
			if(areaid[0] == ATMPoint[i]) {
				...
				format(szMiscArray, sizeof(szMiscArray), "{FF8000}** {C2A2DA}%s tiep can ATM, nhap ma PIN cua ho.", GetPlayerNameEx(playerid));
	...
	switch(dialogid) {
		...
		case ATM_AMOUNT: {
			...
			if(GetPVarType(playerid, "ATMWithdraw")) {
				...
				if(iAmount < 1) {
					SendClientMessageEx(playerid, COLOR_WHITE, "  So tien am khong the rut!");
					return ShowATMMenu(playerid, 1);
				}

				if(iAmount > PlayerInfo[playerid][pAccount]) {
					SendClientMessageEx(playerid, COLOR_WHITE, "  Ban dang co rut nhieu hon so tien ban co!");
					return ShowATMMenu(playerid, 1);
				}
				...
				format(szMiscArray, sizeof(szMiscArray), "  Ban da rut tien $%s tu tai khoan cua ban. ", number_format(iAmount));
				...
				if(PlayerInfo[playerid][pDonateRank] == 0) {
					...
					format(szMiscArray, sizeof(szMiscArray), "  Ban da bi tinh phi rut tien 3 phan tram: -$%d.", fee);
					SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
				}
				...
				if(iAmount < 1) {
					SendClientMessageEx(playerid, COLOR_WHITE, "  So tien am khong the gui!");
					return ShowATMMenu(playerid, 2);
				}

				if(iAmount > GetPlayerCash(playerid)) {
					SendClientMessageEx(playerid, COLOR_WHITE, "  Ban dang co gang gui tien nhieu hon ban co!");
					return ShowATMMenu(playerid, 2);
				}
				...
				GivePlayerCash(playerid, -iAmount);
				PlayerInfo[playerid][pAccount] += iAmount; 
				format(szMiscArray, sizeof(szMiscArray), "  Ban da gui $%s vao tai khoan cua ban. ", number_format(iAmount));
				SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
				...
				if(PlayerInfo[playerid][pDonateRank] == 0) {
					...
					if(!Bank_TransferCheck(-fee)) return 1;
					PlayerInfo[playerid][pAccount] -= fee;
					format(szMiscArray, sizeof(szMiscArray), "  Ban da bi tinh phi dat coc 3 phan tram: -$%d.", fee);
					SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
				}
		...
		case ATM_TRANSFER_TO: {
			...
			if(!IsPlayerConnected(id) || !gPlayerLogged{id}) {
				SendClientMessageEx(playerid, COLOR_WHITE, "  Ban dang co gang chuyen den nguoi choi khong ket noi!");
		...
		case ATM_TRANSFER_AMT: {
			...
			if(restarting) {
				SendClientMessageEx(playerid, COLOR_GRAD2, "Giao dich hien dang bi vo hieu hoa do may chu duoc khoi dong lai de bao tri.");
				return ShowATMMenu(playerid, 3);
			}
			if(PlayerInfo[playerid][pLevel] < 3) {
				SendClientMessageEx(playerid, COLOR_GRAD1, "   Ban phai it nhat la cap 3 de su dung tinh nang nay!");
				return ShowATMMenu(playerid, 3);
			}
			if(gettime()-GetPVarInt(playerid, "LastTransaction") < 10) {
				SendClientMessageEx(playerid, COLOR_GRAD2, "Ban chi co the thuc hien giao dich cu sao 10 giay, vui long doi!");
			...
			if(PlayerInfo[playerid][pDonateRank] == 0) {
				...
				fee = 3*iAmount/100;
				PlayerInfo[playerid][pAccount] -= fee;
				format(szMiscArray, sizeof(szMiscArray), "  Ban da bi tinh phi chuyen khoan 3 phan tram: -$%d.", fee);
				SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
			}
			...
			format(szMiscArray, sizeof(szMiscArray), "   Ban da chuyen $%s den tai khoan %s's.", number_format(iAmount), GetPlayerNameEx(id));
			SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
			
			format(szMiscArray, sizeof(szMiscArray), "   $%s da duoc chuyen vao tai khoan ngan hang cua ban tu %s.", number_format(iAmount), GetPlayerNameEx(playerid));
...
ShowATMMenu(playerid, menu = 0) {
	...
	if(PlayerInfo[playerid][pFreezeBank] == 1) return ShowPlayerDialogEx(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, szTitle, "Tai sai cua ban da bi dong bang! Lien he Tu Phap", "Ok", "");

	switch(menu) {

		case 0: { // main menu
			ShowPlayerDialogEx(playerid, ATM, DIALOG_STYLE_LIST, szTitle, "Rut tien\nGui tien\nChuyen khoan", "Lua chon", "Huy bo");
		}

		case 1: { // iAmount withdraw
			ShowPlayerDialogEx(playerid, ATM_AMOUNT, DIALOG_STYLE_INPUT, szTitle, "Vui long nhap so tien ma ban muon rut tu tai khoan cua minh.", "Rut tien", "Huy bo");
			SetPVarInt(playerid, "ATMWithdraw", 1);
		}

		case 2: { // iAmount deposit
			ShowPlayerDialogEx(playerid, ATM_AMOUNT, DIALOG_STYLE_INPUT, szTitle, "Vui long nhap so tien ban muon gui vao tai khoan cua ban.", "Gui tien", "Huy bo");
			SetPVarInt(playerid, "ATMDeposit", 1);
		}

		case 3: { // transfer to
			ShowPlayerDialogEx(playerid, ATM_TRANSFER_TO, DIALOG_STYLE_INPUT, szTitle, "Vui long nhap ID nguoi choi ma ban muon chuyen khoang.", "Tiep", "Huy bo");
		}

		case 4: { // transfer iAmount
			format(szMiscArray, sizeof(szMiscArray), "Vui long nhap so tien ma ban muon chuyen den {FF0000}%s", GetPlayerNameEx(GetPVarInt(playerid, "ATMTransferTo")));
			ShowPlayerDialogEx(playerid, ATM_TRANSFER_AMT, DIALOG_STYLE_INPUT, szTitle, szMiscArray, "Chuyen khoan", "Huy bo");
		}
	}
