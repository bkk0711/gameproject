...
ShowAccountSettings(playerid, menu = 0) {
	...
	switch(menu) {
		...
		case 2: {
			...
			ShowPlayerDialogEx(playerid, ACCOUNT_CHATBOX, DIALOG_STYLE_TABLIST_HEADERS, "Tuy chon tro chuyen", szMiscArray, "Chon", "Thoat");
		}

		case 3: { // account email 
			ShowPlayerDialogEx(playerid, EMAIL_VALIDATION, DIALOG_STYLE_INPUT, "E-mail Registration", "Vui long nhap E-mail hop le de lien ket tai khoan cua ban.", "Gui", "");
		}

		case 4: { // account password
			ShowPlayerDialogEx(playerid, DIALOG_CHANGEPASS, DIALOG_STYLE_INPUT, "Password Change", "Vui long nhap mat khau moi!\nNhan vien se khong bao gio hoi mat khau ban!", "Thay doi", "Thoat" );
		}
		...
		case 5: { // shop pin
			...
			if(GetPVarInt(playerid, "PinConfirmed")) {
			    ...
			    ShowPlayerDialogEx(playerid, DIALOG_CREATEPIN, DIALOG_STYLE_INPUT, "Change Pin Number", "Nhap so PIN moi de thay doi PIN hien tai.", "Thay doi", "Thoat");
			}
...
CMD:tog(playerid, params[]) {
    ...
    if(isnull(params)) {
        ...
        SendClientMessageEx(playerid, COLOR_GRAD1, "SU DUNG: /tog [option]");
        SendClientMessageEx(playerid, COLOR_GRAD1, "CO SAN: newbie | ooc | whisper | pr | phone | famed | vip | dept | gooc | radio | bug");
        SendClientMessageEx(playerid, COLOR_GRAD1, "CO SAN: biz | staff | advisor | news | chatbox | advisor | points | rf");
        return 1;
    }
    ...
    if(PlayerInfo[playerid][pToggledChats][iChatID] == 0) {
        ...
        format(szMiscArray, sizeof(szMiscArray), "Ban da chuyen doi %s off.", chatname);
        ...
        format(szMiscArray, sizeof(szMiscArray), "Ban da chuyen doi %s on.", chatname);
        SendClientMessage(playerid, COLOR_GRAD1, szMiscArray);
    }
    return 1;
}
