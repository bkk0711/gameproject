	...
	switch(dialogid)
	{
		...
		case DIALOG_SAN_SHOWS:
		{
			...
			if(response)
			{
				if(GetPVarType(playerid, "DirectorChannelID"))
				{
					...
					format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] Giam doc truyen hinh %s da dang nhap vao kenh %i. **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid), GetPVarInt(playerid, "ChannelID_FMEM"));
				...
				{
					SetPVarInt(playerid, "ChannelID_FMEM", listitem+1);
					format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] Quay phim %s da dang nhap vao kenh %i. **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid), GetPVarInt(playerid, "ChannelID_FMEM"));
					SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
				...
				{
					SetPVarInt(playerid, "ChannelID_FMEM", listitem+1);
					format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] Dan chuong trinh %s da dang nhap vao kenh %i. **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid), GetPVarInt(playerid, "ChannelID_FMEM"));
					SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
		...
		case DIALOG_SAN_SHOWS_EDIT2:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_NAME, DIALOG_STYLE_INPUT, "Chuong trinh Sanews | Ten", "Nhap ten moi cua chuong trinh.", "Nhap", "Huy bo");
					}
					case 1:
					{
						ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_HOST, DIALOG_STYLE_INPUT, "Chuong trinh Sanews | Dan chuong trinh", "Nhap ten cua nguoi dan chuong trinh(s).", "Nhap", "Huy bo");				
					}
					case 2:
					{
						ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_GLOBAL, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Kenh toan cau", "Kich hoat kenh toan cau\nVo hieu hoa kenh toan cau", "Nhap", "Huy bo");				
					}
					case 3:
					{
						ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_TYPE, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Loai chuong trinh", "Chuong trinh TV\nChuong trinh Radio\nChuong trinh tuy chinh", "Nhap", "Huy bo");				
					}
					case 4:
					{
						ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_TOGGLE, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Bat dau / Dung phat song", "{EEFFEE}BAU DAU {FFFFFF}Phat song\n{FFDDDD}DUNG {FFFFFF}Phat song", "Nhap", "Huy bo");
					}
				}
				return 1;
			}
		...
		case DIALOG_SAN_SHOWS_TOGGLE:
		{
			if(response)
			{
				...
				switch(listitem)
				{
					case 0: 
					{
						if(isnull(SANShows[channel][szChannelType])) return ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_TYPE, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Loai chuong trinh | GIOI THIEU!", "Chuong trinh TV\nChuong trinh Radio\nChuong trinh tuy chinh", "Nhap", "Huy bo");				
				...
				switch(listitem)
				{
					...
					case 2:
					{
						...
						format(szMiscArray, sizeof(szMiscArray), "<----- Static Camera Menu\n\
							...
							Ket thuc va phat song");
						ShowPlayerDialogEx(playerid, DIALOG_SAN_DCAMERAMAN, DIALOG_STYLE_LIST, "SA News | Dynamic Camera Menu", szMiscArray, "Lua chon", "Huy bo");
					}
					case 3:
					{
						return 1;
		...
		case DIALOG_SAN_CAMLIST:
		{
			if(response)
			{
				...
				format(szTitle, sizeof(szTitle), "SAN | Danh sach Camera | Chuong trinh %i", GetPVarInt(playerid, "ChannelID_FMEM"));
				format(szMiscArray, sizeof(szMiscArray), "Preview %s's camera\nChuong trinh %s's camera", GetPlayerNameEx(ListItemTrackID_Cameras[playerid][listitem]), GetPlayerNameEx(ListItemTrackID_Cameras[playerid][listitem]));
				ShowPlayerDialogEx(playerid, DIALOG_SAN_CAMLIST2, DIALOG_STYLE_LIST, szTitle, szMiscArray, "Lua chon", "Huy bo");
			}
			else 
			{
		...
		case DIALOG_SAN_CAMLIST2:
		{
			if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						if(GetPVarType(playerid, "PreviewingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban da xem truoc mot goc camera.");
		...
		case DIALOG_SAN_PSTREAM:
		{
			if(response)
			{
				...
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Nhan 'N' to stop previewing the stream.");
...
public SAN_Viewers(playerid)
{
	...
	if(isnull(szMiscArray)) return SendClientMessage(playerid, COLOR_GRAD1, "Khong co nguoi xem.");
	else ShowPlayerDialogEx(playerid, DIALOG_SAN_VIEWERS, DIALOG_STYLE_LIST, "SAN | Danh sach nguoi xem", szMiscArray, "--", "--");
	return 1;
}
	...
	for(new channel = 1; channel < MAX_SANCHANNELS; channel++)
	{
		switch(choice)
		{
			case 0 .. 1:
			{
				if(!isnull(SANShows[channel][ChannelName]))
				{
					switch(SANShows[channel][ChannelActive]) 
					{
						case 0:
						{
							format(szDialogLine, sizeof(szDialogLine), "{FF3232}[TAT] {FFFFFF}- [CH. %i] || {FFA500}%s {FFFFFF}|| Dan chuong trinh: {32CD80}%s {FFFFFF}|| %s\n", channel, SANShows[channel][ChannelName], SANShows[channel][Hosts], SANShows[channel][szChannelType]);
							format(szMiscArray, sizeof(szMiscArray), "%s%s\n", szMiscArray, szDialogLine);
						
						}
						case 1: 
						{
							format(szDialogLine, sizeof(szDialogLine), "{7CFC00}[BAT]  {FFFFFF}- [CH. %i] || {FFA500}%s {FFFFFF}|| Dan chuong trinh: {32CD80}%s {FFFFFF}|| %s", channel, SANShows[channel][ChannelName], SANShows[channel][Hosts], SANShows[channel][szChannelType]);
							format(szMiscArray, sizeof(szMiscArray), "%s%s\n", szMiscArray, szDialogLine);
						}
					}
				}
				else
				{
					switch(SANShows[channel][ChannelActive]) 
					{
						case 0: format(szMiscArray, sizeof(szMiscArray), "%s{FF3232}[TAT] {FFFFFF}- [CH. %i] || ------ || Dan chuong trinh: -- || --\n", szMiscArray, channel);
						case 1: format(szMiscArray, sizeof(szMiscArray), "%s {7CFC00}[BAT]  {FFFFFF}- [CH. %i] || ------ || Dan chuong trinh: -- || --\n", szMiscArray, channel);
					}
				}
			}
			case 2 .. 3:
			{
				...
				format(szMiscArray, sizeof(szMiscArray),"%sDang nhap vao kenh\nDanh sach chuong trinh\nChinh sua chuong trinh\nDang xuat khoi he thong", szMiscArray);
			}
		}
	}
	switch(choice)
	{
		case 0: return ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Dieu chinh", szMiscArray, "Lua chon", "Huy bo");
		case 1: return ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_EDIT, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Chinh sua bang dieu khien", szMiscArray, "Lua chon", "Huy bo");
		case 2: return ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_MENU, DIALOG_STYLE_LIST, "Chuong trinh Sanews | Menu", szMiscArray, "Lua chon", "Huy bo");
		case 3:
		{
			format(szTitle, sizeof(szTitle), "SAN | Chinh sua: Chuong trinh %i | %s", GetPVarInt(playerid, "SAN_EditingChannelID"), SANShows[GetPVarInt(playerid, "SAN_EditingChannelID")][ChannelName]);
			format(szMiscArray, sizeof(szMiscArray), "Chinh sua ten chuong trinh\nChinh sua dan chuong trinh\nChinh sua dinh tuyen kenh toan cau\nChinh sua loai chuong trinh\nBat dau / Dung Phat song");
			ShowPlayerDialogEx(playerid, DIALOG_SAN_SHOWS_EDIT2, DIALOG_STYLE_LIST, szTitle, szMiscArray, "Lua chon", "Huy bo");
		}
	}
	return 1;
}
...
public SAN_Broadcast(playerid, channel)
{
	...
	{
		if(shutdown == 1) return SendClientMessage(playerid, COLOR_LIGHTBLUE, "He thong tin tuc hien dang ngung hoat dong." );
		{
			...
			if(SANShows[channel][ChannelActive] == 1)
			{
				...
				// TVObjects_Spawn();
 				...
 				broadcasting = 1;
 				format(szMiscArray, sizeof(szMiscArray), "** %s da bat dau he thong kenh chuong trinh %i. **", GetPlayerNameEx(playerid), GetPVarInt(playerid, "ChannelID_FMEM"));
				SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
				format(szMiscArray, sizeof(szMiscArray), "[SAN]: Chung toi se phat song %s LIVE voi %s tren kenh %i. (( Dung /shows de dieu chinh! ))", SANShows[channel][ChannelName], SANShows[channel][Hosts], channel); // This announces the broadcast to all players.
			...
			{
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong con phat song");
				...
				foreach(new i : Player)
				{
					if(GetPVarType(i, "WatchingTV"))
					{
						...
						SendClientMessage(playerid, COLOR_GRAD1, "Chuong trinh da duoc ket thuc boi cac nha san xuat.");
						SendClientMessage(playerid, COLOR_GRAD1, "Lay vi tri cu cua nhan vat cua ban...");
...
public SAN_ListenRadio(playerid, channel)
{
	...
	if(!GetPVarType(playerid, "WatchingTV"))
	{
		...
		{
			if(GetPVarType(playerid, "PreviewingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the su dung lenh nay trong khi ban dang quay camera.");
			if(SANShows[channel][ChannelActive] == 0) return SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong co gi tren kenh TV do!"); // This makes sure that they can't do /watchtv when there's no one broadcasting.
			format(szMiscArray, sizeof(szMiscArray), "* %s dieu chinh trong %s tren [SAN] chuong trinh %i", GetPlayerNameEx(playerid), SANShows[GetPVarInt(playerid, "ChannelID")][ChannelName], GetPVarInt(playerid, "ChannelID"));
			ProxDetector(20.0, playerid, szMiscArray, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Go /shows mot lan nua de ngung nghe phat song.");
		...
		{
			SendClientMessage(playerid, COLOR_GRAD1, "[SAN] Da xay ra loi. Vui long thu lai sau.");
		}
	}
	else
	{
		format(szMiscArray, sizeof(szMiscArray), "* %s dung theo doi %s mo [SAN] Chuong trinh %i", GetPlayerNameEx(playerid), SANShows[GetPVarInt(playerid, "ChannelID")][ChannelName], GetPVarInt(playerid, "ChannelID"));
...
public SAN_WatchTV(playerid, channel)
{
	...
	for(new i; i < MAX_HOUSES; ++i)
	{
		if(IsPlayerInRangeOfPoint(playerid, 50, HouseInfo[i][hInteriorX], HouseInfo[i][hInteriorY], HouseInfo[i][hInteriorZ]) && GetPlayerVirtualWorld(playerid) == HouseInfo[i][hIntVW] && GetPlayerInterior(playerid) == HouseInfo[i][hIntIW])
		{
			if(GetPVarType(playerid, "ChannelID"))
			{
				if(GetPVarType(playerid, "PreviewingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the su dung lenh nay trong khi quay camera.");
				if(SANShows[channel][ChannelActive] == 0) return SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong co gi tren kenh TV do!"); // This makes sure that they can't do /watchtv when there's no one broadcasting.
				format(szMiscArray, sizeof(szMiscArray), "* %s bat dau xem TV.", GetPlayerNameEx(playerid));
				ProxDetector(15.0, playerid, szMiscArray, COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Go /shows de dung xem TV.");
	...
	SendClientMessage(playerid, COLOR_GRAD1, "Ban phai o trong mot ngoi nha de xem TV.");
	return 1;
}
...
public SAN_Process_Login(playerid)
{
	...
	{
		format(szMiscArray, sizeof(szMiscArray), "___________________________ {32CD80} DANG NHAP | SAN {FFFFFF}____________________\nNhan vao day de dang nhap vao kenh.");
		ShowPlayerDialogEx(playerid, DIALOG_SAN_LOGIN, DIALOG_STYLE_LIST, "SAN | Bang dang nhap chuong trinh", szMiscArray, "Lua chon", "Huy bo");
		return 0;
	}
	return 1;
}
...
public SAN_Process_Logout(playerid)
{
	...
	if(GetPVarType(playerid, "ChannelID_FMEM"))
	{
		...
		format(szMiscArray, sizeof(szMiscArray), "** %s dang xuat khoi kenh %i. **", GetPlayerNameEx(playerid), GetPVarInt(playerid, "ChannelID_FMEM"));
		SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
		DeletePVar(playerid, "ChannelID");
		DeletePVar(playerid, "ChannelID_FMEM");
	...
	{
		SendClientMessage(playerid, COLOR_GRAD1, "Ban chua dang nhap vao kenh.");
	}
}
...
public SAN_Process_ListCameras(playerid, channel)
{
		...
		if(GetPVarInt(i, "ChannelID_FMEM") == GetPVarInt(playerid, "ChannelID_FMEM"))
		{
			if(CameramanFloats[GetPVarInt(i, "ChannelID_FMEM")][i][1] != 0.0 || ICCameramanFloats[GetPVarInt(i, "ChannelID_FMEM")][i][0] != 0.0 && ICCameramanFloats[GetPVarInt(i, "ChannelID_FMEM")][i][3] != 0.0)
			{
				switch(GetPVarInt(i, "iCameraman_Type"))
				{
					case 1:
					{
						format(szMiscArray, sizeof(szMiscArray), "%s Ten: %s || {FFFF99}TINH TRANG\n", szMiscArray, GetPlayerNameEx(i));
					}
					case 2:
					{
						format(szMiscArray, sizeof(szMiscArray), "%s Ten: %s || {CCFFCC}DYNAMIC{FFFFFF}\n", szMiscArray, GetPlayerNameEx(i));
					}
					case 3:
					{
						format(szMiscArray, sizeof(szMiscArray), "%s Ten: %s || {BBEEFF}XE{FFFFFF}\n", szMiscArray, GetPlayerNameEx(i));
					}
				}
			}
			if(i == iLastCameraman[channel])
			{
				format(szMiscArray, sizeof(szMiscArray), "%s Ten: %s || {4C4CFF}PHAT SONG\n", szMiscArray, GetPlayerNameEx(i));
			}
	...
	{
		format(szTitle, sizeof(szTitle), "SAN | Danh sach Camera | Chuong trinh %i", GetPVarInt(playerid, "ChannelID_FMEM"));
		ShowPlayerDialogEx(playerid, DIALOG_SAN_CAMLIST, DIALOG_STYLE_LIST, szTitle, szMiscArray, "Lua chon", "Huy bo");
	}
	else SendClientMessage(playerid, COLOR_GRAD1, "Hien tai khong co Camera hoat dong.");
	return 1;
}
...
public SAN_Process_Cameraman(playerid, choice, channel)
{
	...
	switch(choice)
	{
		...
		{
			if(GetPVarInt(playerid, "WatchingTV") == 1) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the dat goc khi ban dang xem TV.");
			TogglePlayerControllable(playerid, 0);
			if(!GetPVarType(playerid, "PreviewingTV"))
			{
			...
			SAN_ShowTextDraws(playerid);
			SetPVarInt(playerid, "iCameraman_Type", 1);
			format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] Cameraman %s dang cai dat goc camera... **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid));
			SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "Dung 'N' de huy bo. Dung -- '/cameraman' -> Phat song -- de phat song goc hien tai.");
		}
		case SAN_CAMERAMAN_PREVIEW:
		{
			if(iLastCameraman[channel] == -1)
			{
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong co camera hoat dong.");
				return 1;
			}
		...
		case SAN_CAMERAMAN_STOP:
		{
			...
			else return SendClientMessage(playerid, COLOR_GRAD1, "Ban da khong xem truoc mot goc camera.");
		}
		case SAN_CAMERAMAN_BROADCAST:
		{
			if(GetPVarType(playerid, "WatchingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the dat goc khi ban dang xem TV.");
			// SAN_CreateCamera(playerid, channel);
			iLastCameraman[channel] = playerid;
			SANShows[channel][san_iVehicleID] = 0;
			cameraangle = 1;
			SetPVarInt(playerid, "iCameraman_Type", 1);
			format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] %s dang phat goc quay anh. **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid));
			SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
			SAN_Process_Camera(playerid, channel);
			SAN_Process_StopPreview(playerid);
		}
		case SAN_CAMERAMAN_BIRDSEYE:
		{
			...
			format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] %s Thay doi goc camera thanh 'Bird's Eye' view. **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid));
			SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
			cameraangle = 2;
			SAN_Process_Camera(playerid, channel);
		}
		case SAN_CAMERAMAN_VEHICLE: // Nifty stuff here: The camera is set on the wing/foot of the aircraft. I used the spectate function, and it's working brilliantly (it's really smooth).
		{
			if(IsPlayerInAnyVehicle(playerid))
			{
				...
				cameraangle = 7;
				format(szMiscArray, sizeof(szMiscArray), "** [CH. %i] %s dang phat song tu &s  **", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid), GetVehicleName(GetVehicleModel(GetPVarInt(iLastCameraman[channel], "SAN_iVehCam"))));
				SAN_SendRadioMessage(playerid, RADIO, szMiscArray);
				SAN_Process_Camera(playerid, channel);
			}
			else
			{
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Ban phai o trong mot chiec xe de su dung lenh nay!");
				return 1;
			}
		}
		...
		{
			if(GetPVarType(playerid, "WatchingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the dat goc khi ban dang xem TV.");
			if(GetPVarType(playerid, "PreviewingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban da xem truoc goc may anh.");
			GetPlayerPos(playerid, BroadcastFloats[playerid][1], BroadcastFloats[playerid][2], BroadcastFloats[playerid][3]);
			BroadcastLastVW[playerid] = GetPlayerVirtualWorld(playerid);
			BroadcastLastInt[playerid] = GetPlayerInterior(playerid);
			GetPlayerFacingAngle(playerid, BroadcastFloats[playerid][0]);
		...
		{
			if(ICCameramanFloats[channel][playerid][0] == 0.0 || ICCameramanFloats[channel][playerid][3] == 0.0) return SendClientMessage(playerid, COLOR_GRAD1, "De phat song, ban phai thiet lap diem A va B truoc.");
			SetPVarInt(playerid, "iCameraman_Type", 2);
			SANShows[channel][san_iVehicleID] = 0;
			cameraangle = 4;
...
public SAN_Process_Director(playerid, choice, channel)
{
	...
	switch(choice)
	{
		case SAN_DIRECTOR_PREVIEW:
		{
			if(iLastCameraman[channel] == -1)
			{
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong co nguoi quay phim hoat dong.");
				return 1;
			}
			SAN_Process_StartPreview(playerid, iLastCameraman[channel]);
		}
		case SAN_DIRECTOR_STOP:
		{
			if(GetPVarType(playerid, "PreviewingTV"))
			{
				if(iLastCameraman[channel] == -1) SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong co nguoi quay phim hoat dong.");
				SAN_Process_StopPreview(playerid);
			}
			else return SendClientMessage(playerid, COLOR_GRAD1, "Ban da khong xem truoc mot goc camera.");
		}
...
{
	szMiscArray[0] = 0;
	if(GetPVarType(playerid, "WatchingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong the xem truoc mot goc khi ban dang xem TV.");
	if(GetPVarType(playerid, "PreviewingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Ban da xem truoc mot goc camera.");
	BroadcastLastVW[playerid] = GetPlayerVirtualWorld(playerid);
	BroadcastLastInt[playerid] = GetPlayerInterior(playerid);
	SetPVarInt(playerid, "PreviewingTV", 1);
	format(szMiscArray, sizeof(szMiscArray), "Ban dang xem %s's camera angle.", GetPlayerNameEx(giveplayerid));
	SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
	GetPlayerPos(playerid, BroadcastFloats[playerid][1], BroadcastFloats[playerid][2], BroadcastFloats[playerid][3]);
	GetPlayerFacingAngle(playerid, BroadcastFloats[playerid][0]);
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Dung 'N' de huy bo. Dung -- '/bdirector' hoac '/cameraman' -> Phat song -- de phat song goc hien tai.");
	SAN_ShowTextDraws(playerid);
	DeletePVar(playerid, "ListItemID_Cameras");
	TogglePlayerControllable(playerid, 0);
	TogglePlayerSpectating(playerid, 1);
...
public SAN_Process_Camera(playerid, channel)
{
	...
	DeletePVar(playerid, "ListItemID_Cameras");
	format(szMiscArray, sizeof(szMiscArray), "*SAN | Camera*\n {FFFFFF}Chuong trinh: %i\nNha dieu hanh: %s", channel, GetPlayerNameEx(iLastCameraman[channel]));
	DestroyDynamic3DTextLabel(SANShows[channel][camera]);
...
UpdateSANewsBroadcast(channel, ishutdown = 0)
{
	...
	{
		format(szMiscArray, sizeof(szMiscArray), "{FFFFFF}GIAM SAT HE THONG CHINH\n{32CD80}SAN | Chuong trinh {FFFFFF}%i\n{32CD80}Ten: {FFFFFF}%s\n{32CD80}Dan chuong trinh: {FFFFFF}%s\n{32CD80}View: {FF4C4C}Khong phat song\n{32CD80}Xep hang nguoi xem: {FFFFFF}- **", channel, SANShows[channel][ChannelName], SANShows[channel][Hosts], SANShows[channel][Hosts]);
	}
	else
	{
		format(szMiscArray, sizeof(szMiscArray), "{FFFFFF}GIAM SAT HE THONG CHINH\n{32CD80}SAN | Chuong trinh {FFFFFF}%i\n{32CD80}Ten: {FFFFFF}%s\n{32CD80}Dan chuong trinh: {FFFFFF}%s\n{32CD80}Hien tai: {7CFC00}Phat song\n{32CD80}Xep hang nguoi xem: {FFFFFF}%i", channel, SANShows[channel][ChannelName], SANShows[channel][Hosts], SANShows[channel][san_iRatings]);
	}
	UpdateDynamic3DTextLabelText(SANShows[channel][SANews3DText][0], COLOR_LIGHTBLUE, szMiscArray);
	if(broadcasting == 0)
	{
		format(szMiscArray, sizeof(szMiscArray), "{32CD80}SAN | Chuong trinh {FFFFFF}%i\n{32CD80}Ten: {FFFFFF}%s\n{32CD80}Hien tai: {FF4C4C}Khong phat song\n{32CD80}Xep hang nguoi xem: {FFFFFF} **", channel, SANShows[channel][ChannelName]);
	}
	else
	{
		format(szMiscArray, sizeof(szMiscArray), "{32CD80}SAN | Chuong trinh {FFFFFF}%i\n{32CD80}Ten: {FFFFFF}%s\n{32CD80}Hien tai: {7CFC00}Phat song\n{32CD80}Xep hang nguoi xem: {FFFFFF}%i", channel, SANShows[channel][ChannelName], SANShows[channel][san_iRatings]);
	}
...
SAN_Stop(i)
{
	if(GetPVarType(i, "WatchingTV"))
	{
		...
		DeletePVar(i, "WatchingTV");
		DeletePVar(i, "ChannelID");
		SendClientMessage(i, COLOR_GRAD1, "Lay vi tri cu cua nhan vat cua ban...");
		SetTimerEx("SAN_SetPos", 5000, false, "i", i);
	...
	if(GetPVarInt(playerid, "TVOffer") < MAX_PLAYERS) {
		if(IsPlayerConnected(GetPVarInt(playerid, "TVOffer"))) {
			if (ProxDetectorS(5.0, playerid, GetPVarInt(playerid, "TVOffer"))) {
				SendClientMessage(GetPVarInt(playerid, "TVOffer"), COLOR_LIGHTBLUE, "* Nguoi choi da chap nhan loi moi cua ban.");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Chuong trinh da bat dau! Dung '/mic' de noi. (Dung /shows de dung).");
				...
				SendClientMessage(playerid, COLOR_GREY, "   Ban o qua xa phong vien!");
				return 1;
			}
		}
	}
...
{
	if(!IsAReporter(playerid)) return SendClientMessage(playerid, COLOR_GRAD1, "     Ban khong phai la phong vien.");
	if(!SAN_RankCheck(playerid, 1)) return 1;
	new giveplayerid;
	if(sscanf(params, "u", giveplayerid)) return SendClientMessage(playerid, COLOR_WHITE, "Su dung: /showkick [playerid]");
	if(GetPVarInt(giveplayerid, "TalkingTV") == GetPVarInt(playerid, "TalkingTV")) return SendClientMessage(playerid, COLOR_GRAD1, "Nguoi choi do khong co mat trong chuong trinh.");
	SetPVarInt(giveplayerid, "TalkingTV", INVALID_PLAYER_ID);
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da kicked thanh cong nguoi choi tu chuong trinh.");
	SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, "Ban da bi kicked tu chuong trinh.");
	return 1;
}
...
CMD:stopnews(playerid, params[])
{
	if(IsAReporter(playerid))
	{
		...
		{
			shutdown = 1;
			SendClientMessage(playerid, COLOR_WHITE, "Ban vua tat toan bo he thong tin tuc va se ket thuc moi thu trong progres..." );
			if(broadcasting == 1)
			{
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Phat song vua duoc tat..");
				broadcasting = 0;
				for(new channel; channel < MAX_SANCHANNELS; channel ++)
				{
					UpdateSANewsBroadcast(channel, 1);
			...
			foreach(new i : Player)
			{
				...
				{
					SendClientMessage(i, COLOR_LIGHTBLUE, "* Cuoc tro chuyen truc tiep da ket thuc.");
					SendClientMessage(TalkingLive[i], COLOR_LIGHTBLUE, "* Cuoc tro chuyen truc tiep da ket thuc.");
					TogglePlayerControllable(i, 1);
					TogglePlayerControllable(TalkingLive[i], 1);
					TalkingLive[TalkingLive[i]] = INVALID_PLAYER_ID;
					TalkingLive[i] = INVALID_PLAYER_ID;
					SendClientMessage(i, COLOR_LIGHTBLUE, "* Live vua bi tat..");
				}
				else if(GetPVarInt(i, "TalkingTV") != INVALID_PLAYER_ID)
				{
					SendClientMessage(i, COLOR_LIGHTBLUE, "* Chuon trinh ket thuc.");
					SendClientMessage(GetPVarInt(i, "TalkingTV"), COLOR_LIGHTBLUE, "* Chuong trinh ket thuc.");
					SetPVarInt(GetPVarInt(i, "TalkingTV"), "TalkingTV", INVALID_PLAYER_ID);
					SetPVarInt(i, "TalkingTV", INVALID_PLAYER_ID);
					SendClientMessage(i, COLOR_LIGHTBLUE, "* Chuong trinh vua duoc tat...");
				}
			}
		}
		else
		{
			shutdown = 0;
			SendClientMessage(playerid, COLOR_WHITE, "Ban vua bat he thong tin tuc tro lai. " );
		}
	}
	return 1;
}
...
CMD:broadcast(playerid, params[])
{
	...
	{
		SendClientMessage(playerid, COLOR_GRAD1, "     Ban khong phai la phong vien.");
	}
	return 1;
}
...
CMD:news(playerid, params[])
{
	...
	if(IsAReporter(playerid))
	{
		...
		if(shutdown == 1) return SendClientMessage(playerid, COLOR_WHITE, "He thong tin tuc hien dang ngung hoat dong." );
		if(isnull(params)) return SendClientMessage(playerid, COLOR_GREY, "Su dung: /news [text]");
		...
		if(IsANewsCar(newcar) || IsPlayerInRangeOfPoint(playerid, 35.0, 664.6821,-2.3673,1101.2085) || IsPlayerInRangeOfPoint(playerid, 35.0, 651.1910,-19.9795,1101.2200) || IsPlayerInRangeOfPoint(playerid, 35.0, 646.7300,-6.6966,1101.2085))
		{
			...
			{
				SendClientMessage(playerid, COLOR_GRAD2, "Ban phai co it nhat cap bac 1.");
			}
			else
			{
				format(szMiscArray, sizeof(szMiscArray), "NR %s: %s", GetPlayerNameEx(playerid), params);
				OOCNews(COLOR_NEWS, szMiscArray);
				format(szMiscArray, sizeof(szMiscArray), "[TIN TUC] NR %s: %s", GetPlayerNameEx(playerid), params);
				GroupLog(PlayerInfo[playerid][pMember], szMiscArray);
			}
		}
		else
		{
			SendClientMessage(playerid, COLOR_GREY, "Ban khong o trong mot chiec xe Sanews.");
			return 1;
		}
	}
	else
	{
		SendClientMessage(playerid, COLOR_GREY, "     Ban khong phai la phong vien.");
	}
	return 1;
}
...
CMD:setchannel(playerid, params[])
{
	...
	else SendClientMessage(playerid, COLOR_GREY, "     Ban khong phai la phong vien.");
	return 1;
}
...
CMD:mic(playerid, params[])
{
	szMiscArray[0] = 0;
	if(shutdown == 1) return SendClientMessage(playerid, COLOR_WHITE, "He thong tin tuc hien dang ngung hoat dong." );
	if(broadcasting == 0) return SendClientMessage(playerid, COLOR_LIGHTBLUE, "* Khong ai phat song!");
	if(isnull(params)) return SendClientMessage(playerid, COLOR_WHITE, "Su dung: /mic [text]");
		...
		switch(SANShows[GetPVarInt(playerid, "ChannelID_FMEM")][Global])
		{
			case 0:
			{
				switch(SANShows[GetPVarInt(playerid, "ChannelID_FMEM")][ChannelType])
				{
					case 0:
					{
						format(szMiscArray, sizeof(szMiscArray), "[SAN] Dan chuong trinh %s: %s", GetPlayerNameEx(playerid), params);
			...
			case 1:
			{
				switch(SANShows[GetPVarInt(playerid, "ChannelID_FMEM")][ChannelType])
				{
					case 0:
					{
						format(szMiscArray, sizeof(szMiscArray), "[CH. %i] Dan chuong trinh TV %s: %s", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid), params);
						TVNews(COLOR_NEWS, szMiscArray, GetPVarInt(playerid, "ChannelID_FMEM"));
					}
					case 1:
					{
						format(szMiscArray, sizeof(szMiscArray), "[CH. %i] Dan chuong trinh %s: %s", GetPVarInt(playerid, "ChannelID_FMEM"), GetPlayerNameEx(playerid), params);
						TVNews(COLOR_SANRADIO, szMiscArray, GetPVarInt(playerid, "ChannelID_FMEM"));					
					}
					case 2:
					{
	...
	{
		SendClientMessage(playerid, COLOR_GRAD1, "Ban chua dang nhap vao kenh. Vui long dang nhap ngay."); 
	...
	if(GetPVarInt(playerid, "ChannelID"))
	{
			...
			switch(SANShows[GetPVarInt(playerid, "ChannelID")][Global])
			{
				case 0:
				{
					switch(SANShows[GetPVarInt(playerid, "ChannelID_FMEM")][ChannelType])
					{
						case 0:
						{
							format(szMiscArray, sizeof(szMiscArray), "[SAN] Khach %s: %s", GetPlayerNameEx(playerid), params);
				...
				case 1:
				{
					switch(SANShows[GetPVarInt(playerid, "ChannelID")][ChannelType])
					{
						case 0:
						{
							format(szMiscArray, sizeof(szMiscArray), "[CH. %i] TV Khach %s: %s", GetPVarInt(playerid, "ChannelID"), GetPlayerNameEx(playerid), params);
							TVNews(COLOR_NEWS2, szMiscArray, GetPVarInt(playerid, "ChannelID"));
						}
						case 1:
						{
							format(szMiscArray, sizeof(szMiscArray), "[CH. %i] Radio Khach %s: %s", GetPVarInt(playerid, "ChannelID"), GetPlayerNameEx(playerid), params);
							TVNews(COLOR_SANRADIO2, szMiscArray, GetPVarInt(playerid, "ChannelID"));				
						}
						case 2:
						{
							format(szMiscArray, sizeof(szMiscArray), "[CH. %i] [SAN] Khach %s: %s", GetPVarInt(playerid, "ChannelID"), GetPlayerNameEx(playerid), params);
							TVNews(COLOR_NEWS2, szMiscArray, GetPVarInt(playerid, "ChannelID"));
						}
					}
				}
		...
		else return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong tham gia chuong trinh.");
	}
	return 1;
}
...
{
	szMiscArray[0] = 0;
	if(!IsAReporter(playerid)) return SendClientMessage(playerid, COLOR_GREY, "     Ban khong phai la phong vien."); 
	if(!SAN_RankCheck(playerid, 1)) return 1;
	if(shutdown == 1) return SendClientMessage(playerid, COLOR_WHITE, "He thong tin tuc hien dang ngung hoat dong." );
	if(GetPVarType(playerid, "ChannelID_FMEM"))
	{
		new giveplayerid;
		if(sscanf(params, "u", giveplayerid)) return SendClientMessage(playerid, COLOR_WHITE, "Su dung: /showinvite [playerid]");
		if (IsPlayerConnected(giveplayerid))
		{
			if (ProxDetectorS(5.0, playerid, giveplayerid))
			{
				if(PlayerInfo[giveplayerid][pLiveBanned] == 1) return SendClientMessage(playerid, COLOR_GREY, "That player is banned from the aircraft.");
				...
				else
				{
					...
					{
						if(giveplayerid == playerid) return SendClientMessage(playerid, COLOR_GREY, "Su dung: '/mic' de noi chuyen chuong trinh TV!");
						format(szMiscArray, sizeof(szMiscArray), "* Ban da de nghi %s Tham gia chuong trinh TV.", GetPlayerNameEx(giveplayerid));
						SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
						format(szMiscArray, sizeof(szMiscArray), "* %s de nghi ban tham gia chuong trinh TV, go /acceptshow de chap nhan.", GetPlayerNameEx(playerid));
						SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
					}
					else
					{
						if(giveplayerid == playerid) return SendClientMessage(playerid, COLOR_GREY, "Su dung: '/mic' de noi chuyen chuong trinh Radio!");
						format(szMiscArray, sizeof(szMiscArray), "* Ban duoc cung cap %s Tham gia chuong trinh Radio.", GetPlayerNameEx(giveplayerid));
						SendClientMessage(playerid, COLOR_LIGHTBLUE, szMiscArray);
						format(szMiscArray, sizeof(szMiscArray), "* %s de nghi ban tham gia chuong trinh Radio, go /acceptshow de chap nhan.", GetPlayerNameEx(playerid));
						SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, szMiscArray);
					}
		...
		{
			SendClientMessage(playerid, COLOR_GREY, "Nguoi choi khong hop le.");
		}
	}
	else SendClientMessage(playerid, COLOR_GRAD1, "Ban chua dang nhap vao kenh. (Dung '/setchannel' de dang nhap).");
	return 1;
}
...
CMD:togshow(playerid, params[])
{
	if (!gNews[playerid])
	{
		...
		SendClientMessage(playerid, COLOR_GRAD2, "Da vo hieu hoa tro chuyen TV.");
	}
	else
	{
		gNews[playerid] = 0;
		PlayerInfo[playerid][pToggledChats][1] = 0;
		SendClientMessage(playerid, COLOR_GRAD2, "Da kich hoat tro chuyen TV.");
	}
	return 1;
}
...
CMD:cameraman(playerid, params[])
{
	if(IsAReporter(playerid))
	{
		...
		for(new channel; channel < MAX_SANCHANNELS; channel++)
		{
			if(shutdown == 1)
			{
				format(szMiscArray, sizeof(szMiscArray), "__________________________ {FF3232}SYSTEM OFFLINE {FFFFFF}________________\nHoat dong: {FFA500}--- {FFFFFF}|| {32CD80}Chuong trinh ---{FFFFFF} (nhan vao de thay doi)\n");
				break;
			}
...
CMD:bdirector(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pRank] < 3) return SendClientMessage(playerid, COLOR_GRAD1, "You must be at least Rank 3 to use the Broadcast Director system.");
	{
		...
		for(new channel; channel < MAX_SANCHANNELS; channel++)
		{
			if(shutdown == 1)
			{
				format(szMiscArray, sizeof(szMiscArray), "__________________________ {FF3232}SYSTEM OFFLINE {FFFFFF}________________\nHoat dong: {FFA500}--- {FFFFFF}|| {32CD80}Chuong trinh ---{FFFFFF} (nhan de thay doi)\n");
				break;
			}
			if(GetPVarInt(playerid, "ChannelID_FMEM") == channel)
			{
				if(SANShows[channel][ChannelActive] == 1)
				{
					format(szMiscArray, sizeof(szMiscArray), "______________________________ {FF4C4C}ON AIR {FFFFFF}______________________________\nHoat dong: {FFA500}%s {FFFFFF}|| {32CD80}Chuong trinh %i{FFFFFF} (nhan de thay doi)\n", SANShows[channel][ChannelName], GetPVarInt(playerid, "ChannelID_FMEM"));
					break;
				}
				if(SANShows[channel][ChannelActive] == 0)
				{
...
CMD:channeldesc(playerid, params[])
{
	...
	if(IsAReporter(playerid))
	{
		...
		if(!isnull(params))
		{
			format(szMiscArray, sizeof(szMiscArray), "Chuong trinh TV duoc dat thanh: %s", params);
			SendClientMessage(playerid, COLOR_WHITE, szMiscArray);
			TextDrawSetString(TV_text[4], params);
		}
		else
		{
			SendClientMessage(playerid, COLOR_WHITE, "Su dung: /channeldesc [mieu ta]");
		}
	}
	else
	{
...
public SAN_InitTextDraws()
{
	...
	TV_text[0] = TextDrawCreate(0.000000, 120.000000, "tv-trai");
	...
	TV_text[1] = TextDrawCreate(638.000000, 85.000000, "tv-phai");
	...
	TV_text[2] = TextDrawCreate(313.000000, 439.000000, "tv-xuong");
	...
	TV_text[3] = TextDrawCreate(313.000000, -1.000000, "tv-len");
	...
	TV_text[6] = TextDrawCreate(317.000000, 438.000000, "Chuong trinh 1");
...
// Reset TV Textdraws (/director stopcameras).
	...
	TV_text[12] = TextDrawCreate(251.999984, 218.312500, "Vui long doi...");
	...
	TV_text[13] = TextDrawCreate(190.800003, 205.187500, "Chung toi hien dang gap kho khan ve ky thuat.");
