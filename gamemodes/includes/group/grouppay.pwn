...
GovEditGroupBudget(playerid)
{
	...
	format(szTitle, sizeof(szTitle), "{%s}%s {FFFFFF}| Tong quan ve ngan sach", Group_NumToDialogHex(arrGroupData[PlayerInfo[playerid][pLeader]][g_hDutyColour]), arrGroupData[PlayerInfo[playerid][pLeader]][g_szGroupName]);
	return ShowPlayerDialogEx(playerid, DIALOG_GROUP_GOVLISTPAY, DIALOG_STYLE_TABLIST, "Chinh sua ngan sach to chuc", szMiscArray, "Edit", "Cancel");
}
...
PlayerEditGroupPay(playerid)
{
	...
	format(szTitle, sizeof szTitle, "{%s}(%s) {FFFFFF}| Chinh sua tien luong", Group_NumToDialogHex(arrGroupData[iGroupID][g_hDutyColour]), arrGroupData[iGroupID][g_szGroupName]);
...
PayGroupMember(i)
{
	...
	if((0 <= iGroupID < MAX_GROUPS) && 0 <= iRank <= 9 && arrGroupData[iGroupID][g_iPaycheck][iRank] > 0) {
		if(arrGroupData[iGroupID][g_iBudget] > 0) {
			...
			format(szMiscArray,sizeof(szMiscArray)," {%s}%s {FFFFFF}tien luong: $%s", Group_NumToDialogHex(arrGroupData[iGroupID][g_hDutyColour]), arrGroupData[iGroupID][g_szGroupName], number_format(arrGroupData[iGroupID][g_iPaycheck][iRank]));
			SendClientMessageEx(i, COLOR_GRAD2, szMiscArray);
			format(szMiscArray, sizeof(szMiscArray), "%s da thanh toan $%s tu %s's ngan sach.", GetPlayerNameEx(i), number_format(arrGroupData[iGroupID][g_iPaycheck][iRank]), arrGroupData[iGroupID][g_szGroupName]);
			GroupPayLog(iGroupID, szMiscArray);
		}
		else SendClientMessageEx(i, COLOR_RED, "to chuc cua ban dang mac no; khong co tien de tra.");
	}
}
