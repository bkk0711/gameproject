...
hook OnPlayerClickTextDraw(playerid, Text:clickedid) {
	...
	if(clickedid == Furniture_TD[14]) cmd_trogiupnoithat(playerid, "");
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	switch(dialogid) {
		...
		case DIALOG_FURNITURE_BUY: {
			...
			new szTitle[32];
			format(szTitle, sizeof(szTitle), "Do noi that - %s", szFurnitureCategories[listitem]);
			ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_BUYSELECT, DIALOG_STYLE_LIST, szTitle, szMiscArray, "Mua", "Huy bo");
		}
		case DIALOG_FURNITURE_BUYSELECT: {
			...
			if(response) {
				...
				if(PlayerInfo[playerid][pDonateRank] < arrFurnitureCatalog[i][fc_iVIP] && PlayerInfo[playerid][pAdmin] < 4) return SendClientMessageEx(playerid, COLOR_GRAD1, "Cap do VIP cua ban khong du cao de mua do noi that nay.");
				...
				format(szMiscArray, sizeof(szMiscArray), "Ban co muon mua cai nay khong %s gia $%s va %s vat lieu?", GetFurnitureName(arrFurnitureCatalog[i][fc_iModelID]), number_format(GetFurniturePrice(arrFurnitureCatalog[i][fc_iModelID])), number_format(GetFurniturePrice(arrFurnitureCatalog[i][fc_iModelID]) / 10));
				ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_BUYCONFIRM, DIALOG_STYLE_MSGBOX, "Menu Do noi that | Xac nhan mua", szMiscArray, "Mua", "Huy bo");
			}
		...
		case DIALOG_FURNITURE_BUYCONFIRM: {
			...
			if(!response) {
				...
				if(iSlotID > GetMaxFurnitureSlots(playerid) || iSlotID == -1) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong con cho trong nao nua.");
				
				new iPrice = GetFurniturePrice(iModelID);
				if(GetPlayerMoney(playerid) < iPrice) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co du tien de mua cai nay.");
				if(PlayerInfo[playerid][pMats] < floatround((iPrice / 10))) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co du nguyen lieu de lam dieu nay.");
		...
		case DIALOG_FURNITURE_EDIT: {
			...
			if(GetPVarType(playerid, PVAR_FURNITURE_SLOT)) {
				...
				return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT, DIALOG_STYLE_LIST, "Menu Do noi that | Slot", "Slot 1\nSlot 2\nSlot 3\nSlot 4\nSlot 5\n{EE0000}Xoa tat ca", "Lua chon", "Huy bo");
			}
			else {
				...
				if(IsValidFurniture(iHouseID, listitem, 1)) {
					...
					//return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_EDIT, DIALOG_STYLE_LIST, "Menu Do noi that | Edit", "Move position\nChange texture\nChange color", "Lua chon", "Back");
					return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_EDIT, DIALOG_STYLE_LIST, "Menu Do noi that | Chinh sua", "Di chuyen vi tri\nThay doi texture", "Lua chon", "Back");
				}
				else SendClientMessage(playerid, COLOR_GRAD1, "Khong co do noi that trong slot do.");			
			}
		}
		...
		case DIALOG_FURNITURE_PAINT: {
			...
			//return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Slot", "Slot 1\nSlot 2\nSlot 3\nSlot 4\nSlot 5\n{EE0000}Remove All", "Lua chon", "Huy bo");
			SendClientMessageEx(playerid, COLOR_GRAD1, "** Su dung ~k~~GROUP_CONTROL_BWD~ va ~k~~CONVERSATION_NO~ de duyet. Nhan ~k~~CONVERSATION_YES~ de chon.");
			SendClientMessageEx(playerid, COLOR_GRAD1, "** Nhan ~k~~PED_LOOKBEHIND~ de huy bo. Nhan ~k~~PED_DUCK~ de xoa tat ca textures.");
			...
			format(szMiscArray, sizeof(szMiscArray), "Ten: %s", szTextureName[0]);
		...
		case DIALOG_FURNITURE_PAINT2: {
			...
			if(GetPVarType(playerid, "studorfind")) {
				...
				switch(listitem) {
					...
					case 2: {
						...
						return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Lua chon mau", szMiscArray, "Lua chon", "Huy bo"); 
					}
				}
				SetPVarInt(playerid, "processtext", 1);
				return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_INPUT, "Menu Do noi that | Tim kiem Texture", "Vui long chen mot tu khoa cho texture cua ban. De trong neu ban muon duyet tat ca.", "Lua chon", "Huy bo");
			...
			if(GetPVarType(playerid, "processtext")) {
				...
				if(isnull(inputtext) && GetPVarType(playerid, "studio"))  {
					...
					SendClientMessageEx(playerid, COLOR_GRAD1, "[Do noi that]: Hien thi tat ca textures.");
				}
				else {
					...
					for(new i; i < sizeof(arrTextures); ++i) {
						...
						if(strfind(arrTextures[i][text_TXDName], inputtext, true) != -1 || strfind(arrTextures[i][text_TextureName], inputtext, true) != -1) {
							...
							if(x > MAX_SEARCH_TEXTURES) {
								...
								format(szMiscArray, sizeof(szMiscArray), "** Tim kiem cua ban chua nhieu hon ket qua %d. Do do, toi da se duoc hien thi.", MAX_SEARCH_TEXTURES);
				...
				if(GetPVarType(playerid, "studio")) {
					...
					if(PlayerTextureMenuInfo[playerid][ptextm_Menus3D] != -1) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da co mot menu Texture trong World.");
					...
					SendClientMessageEx(playerid, COLOR_GRAD1, "** Next series: ~k~~GROUP_CONTROL_BWD~ va ~k~~SNEAK_ABOUT~ - Previous series:  ~k~~CONVERSATION_NO~ va ~k~~SNEAK_ABOUT~.");
				...
				if(GetPVarType(playerid, "textsearch")) {
					...
					if(!x) return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_INPUT, "Menu Do noi that | Tim kiem Texture", "Tu khoa cua ban khong dua ra bat ky ket qua nao. Vui long thu lai.", "Lua chon", "Huy bo");
					DeletePVar(playerid, "textsearch");
					return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Lua chon Texture", szMiscArray, "Lua chon", "Huy bo");
				}
			}
			...
			if(!GetPVarType(playerid, "textslot")) {
				...
				switch(GetPVarType(playerid, "color")) {
					...
					case 0: {
						...
						//return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Texture Lab", szMiscArray, "Lua chon", "Huy bo");
						SetPVarInt(playerid, "studorfind", 1);
						return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Texturing", "Texture Studio\nSearch Texture", "Lua chon", "Huy bo");
					}
					case 1: return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_INPUT, "Menu Do noi that | Color Lab", "Please enter a HEX color code.", "Lua chon", "Huy bo");
				}
			}
			else {
				...
				SendClientMessageEx(playerid, COLOR_YELLOW, "[Do noi that]: {CCCCCC}Ban da son thanh cong do noi that.");
				return 1;
			}
		}
		case DIALOG_PERMITBUILDER: {
			...
			if(!GetPVarType(playerid, "PRMBLD")) {
				...
				if(!IsPlayerConnected(uPlayer)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh nguoi choi khong hop le id.");

				SetPVarInt(playerid, "PRMBLD", uPlayer);
				format(szMiscArray, sizeof(szMiscArray), "[Do noi that]: {DDDDDD}Nha nao muon cho phep {FFFF00}%s {DDDDDD}xay dung trong?", GetPlayerNameEx(uPlayer));
				SendClientMessage(playerid, COLOR_YELLOW, szMiscArray);
				ShowPlayerDialogEx(playerid, DIALOG_PERMITBUILDER, DIALOG_STYLE_LIST, "Do noi that | Giay phep xay dung", "House 1\nHouse 2\nHouse3", "Lua chon", "Huy bo");
			}
			else {
				...
				switch(listitem) {
					case 0: {
						if(PlayerInfo[playerid][pPhousekey] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co mot ngoi nha trong slot nay."), DeletePVar(playerid, "PRMBLD");
						iHouseID = PlayerInfo[playerid][pPhousekey];
					}
					case 1: {
						if(PlayerInfo[playerid][pPhousekey2] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co mot ngoi nha trong slot nay."), DeletePVar(playerid, "PRMBLD");
						iHouseID = PlayerInfo[playerid][pPhousekey2];
					}
					case 2: {
						if(PlayerInfo[playerid][pPhousekey3] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co mot ngoi nha trong slot nay."), DeletePVar(playerid, "PRMBLD");
						iHouseID = PlayerInfo[playerid][pPhousekey3];
					}
					default: return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban chi dinh nha khong hop le ID."), DeletePVar(playerid, "PRMBLD");
				}
				...
				format(szMiscArray, sizeof(szMiscArray), "%s cap cho ban quyen xay dung nha trong nha cua ho. Ban co quyen thay doi va dat do trong nha.", GetPlayerNameEx(playerid));
				SendClientMessageEx(giveplayerid, COLOR_YELLOW, szMiscArray);
				format(szMiscArray, sizeof(szMiscArray), "Ban da cap %s quyen xay dung nha cua ban.", GetPlayerNameEx(giveplayerid));
				SendClientMessageEx(playerid, COLOR_YELLOW, szMiscArray);
			}
		}
		case DIALOG_FURNITURE_DESCONFIRM: {
			...
			if(response) {
				...
				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da xoa thanh cong cac do dac trong nha.");
			}
			else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da pha huy tat ca do dac trong nha.");
		}
	}
	return 0;
}
...
public OnPlayerSelectDynamicObject(playerid, objectid, modelid, Float:x, Float:y, Float:z) {
	...
	if(GetPVarType(playerid, "copdestroyfur")) {
		...
		if(iHouseID != iData) return SendClientMessageEx(playerid, COLOR_GRAD1, "Doi tuong nay khong phai la mot mon do noi that.");
		...
		if(IsValidFurniture(iHouseID, i, 1)) {
			...
			format(szMiscArray, sizeof(szMiscArray), "** %s pha huy mot mon do noi that.", GetPlayerNameEx(playerid));
			SendGroupMessage(GROUP_TYPE_LEA, COLOR_DBLUE, szMiscArray);
		}
		else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da khong chi dinh mot mon do noi that.");
	}
	if(GetPVarType(playerid, PVAR_FURNITURE)) {
		...
		iData = Streamer_GetIntData(STREAMER_TYPE_OBJECT, objectid, E_STREAMER_EXTRA_ID);
		if(iHouseID != iData) return SendClientMessageEx(playerid, COLOR_GRAD1, "Doi tuong nay khong phai la mon do noi that.");
		for(i = 0; i < MAX_FURNITURE_SLOTS; ++i) if(HouseInfo[iHouseID][hFurniture][i] == objectid) break;

		if(GetPVarType(playerid, "SellFurniture")) { 

			SetPVarInt(playerid, PVAR_FURNITURE_SLOT, i);
			format(szMiscArray, sizeof(szMiscArray), "Ban co chac chan muon ban %s gia $%s?", 
			...
			ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_SELL, DIALOG_STYLE_MSGBOX, "Menu Do noi that | Xac nhan", szMiscArray, "Ban", "Huy bo");
			return 1;
		}
			...
			//ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Texture Slot", "Slot 1\nSlot 2\nSlot 3\nSlot 4\nSlot 5\n{EE0000}Remove All", "Lua chon", "Huy bo");

			// New texture browser on the object:
			SendClientMessageEx(playerid, COLOR_GRAD1, "** Dung ~k~~GROUP_CONTROL_BWD~ va ~k~~CONVERSATION_NO~ de duyet. Nhan ~k~~CONVERSATION_YES~ de chon.");
			SendClientMessageEx(playerid, COLOR_GRAD1, "** Nhan ~k~~PED_LOOKBEHIND~ de huy bo. Nhan ~k~~PED_DUCK~ de xao tat ca textures.");
			PlayerTextureMenuInfo[playerid][ptextm_TPreviewState] = PREVIEW_STATE_SELECT;
			textm_SelectedTile[playerid] = 0;
...
FurnitureMenu(playerid, menu = 0) {
	...
	if(iHouseID == INVALID_HOUSE_ID) {
		...
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong o trong nha.");
	}
	if(!HousePermissionCheck(playerid, iHouseID)) {
		cmd_furniture(playerid, "");
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong duoc phep sua doi do dac cua ngoi nha nay.");
	}
	switch(menu) {
		...
		case 0: {
			...
			/*return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE, DIALOG_STYLE_LIST, "Menu Do noi that", "\
				Buy furniture\n\
				Edit furniture\n\
				Sell furniture\n", "Lua chon", "Huy bo");
			*/
		...
		case 1: { // Buy furniture.
			...
			return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_BUY, DIALOG_STYLE_LIST, "Menu Do noi that | Categories", FurnitureCatalog(), "Lua chon", "Back");
		}
		case 2: { // Edit your own furniture.
			...
			for(new i; i < iMaxSlots; ++i) {
				...
				else format(szMiscArray, sizeof(szMiscArray), "%s[%d] %s\n", szMiscArray, i, "Khong co");
			}
			if(isnull(szMiscArray)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong co noi that.");
			return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_EDIT, DIALOG_STYLE_LIST, "Menu Do noi that | Chinh sua", szMiscArray, "Lua chon", "Huy bo");
		}
		case 3: { // Sell furniture.
			...
			return ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_SELL, DIALOG_STYLE_LIST, "Menu Do noi that | Sell", szMiscArray, "Lua chon", "Huy bo");
			*/
		}
...
SellFurniture(playerid, iHouseID, iSlotID, iPrice) {
	...
	format(szMiscArray, sizeof(szMiscArray), "Ban da ban %s gia $%s", GetFurnitureName(GetDynamicObjectModel(HouseInfo[iHouseID][hFurniture][iSlotID])), number_format(iPrice));
	SendClientMessage(playerid, COLOR_YELLOW, szMiscArray);
...
FurniturePermit(playerid) {
	...
	ShowPlayerDialogEx(playerid, DIALOG_PERMITBUILDER, DIALOG_STYLE_INPUT, "Noi that | Quyen xay dung", "Nhap ID hoac ten cua nguoi ban muon cho phep xay dung trong nha cua ban.", "Lua chon", "Huy bo");
...
public OnLoadFurniture() {
	...
	//return printf("[Do noi that] Loaded %d pieces of furniture from the database.", iCount);
	return 1;
}
...
FurniturePlayerTDInit(playerid) {
	...
	Furniture_PTD[playerid][3] = CreatePlayerTextDraw(playerid, 628.000000, 299.000000, "Ten:");
...
CMD:trogiupnoithat(playerid, params[]) {

	SendClientMessageEx(playerid, COLOR_YELLOW, "[Do noi that] {CCCCCC}/furniture | /myslots | /furnitureresetpos | /permitbuilder | /revokebuilders | /destroyallfurniture | {FF2222}Nhan ~k~~PED_LOOKBEHIND~ (hai lan) de chuyen con tro chuot.");
	SendClientMessageEx(playerid, COLOR_YELLOW, "[Do noi that] {CCCCCC}/unfurnishhouse (xoa noi that GTA:SA mac dinh) | /furnishhouse (them noi that GTA:SA mac dinh)");
	SendClientMessageEx(playerid, COLOR_YELLOW, "[Do noi that] {CCCCCC}Blue House = Mua noi that | Hammer = Che do xay dung (wrench = vi tri, bucket = son mau). | !-icon = Panic Button.");
	SendClientMessageEx(playerid, COLOR_YELLOW, "[Do noi that] {CCCCCC}Dollar Icon = Ban noi that | Green House = Danh sach noi that cua ban. | Red Puppets = Dua quyen xay dung cho nguoi choi.");
	if(IsAdminLevel(playerid, ADMIN_GENERAL, 0)) SendClientMessageEx(playerid, COLOR_YELLOW, "[Do noi that] {FFFF00}/destroyfuniture | /destroyallfurniture | /rehashcatalog");
	return 1;
}
...
CMD:myslots(playerid, params[]) {
	...
	format(szMiscArray, sizeof(szMiscArray), "** Ban co %d slots noi that ban co the dung.", GetMaxFurnitureSlots(playerid));
...
CMD:furniture(playerid, params[]) {
	...
	if(GetPVarType(playerid, PVAR_FURNITURE)) {
		...
		if(!HousePermissionCheck(playerid, i)) return SendClientMessage(playerid, COLOR_GRAD1, "Ban khong duoc phep chinh sua noi that o trong nha nay.");
...
CMD:destroyallfurniture(playerid, params[]) {
	...
	new iHouseID = GetHouseID(playerid);
	if(iHouseID == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong o trong nha.");
	if(HouseInfo[iHouseID][hOwnerID] != GetPlayerSQLId(playerid) && PlayerInfo[playerid][pAdmin] < 2) return SendClientMessageEx(playerid, COLOR_GRAD1, "Chi chu nha moi co the lam dieu nay.");
	ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_DESCONFIRM, DIALOG_STYLE_MSGBOX, "Pha huy noi that", "{FFFFFF}Ban co chac chan muon {FF0000}pha huy vinh vien {FFFFFF}noi that nha cua ban?", "Co", "Khong");
	return 1;
}
...
CMD:unfurnishhouse(playerid, params[]) {
	...
	if(HouseInfo[iHouseID][hOwnerID] != GetPlayerSQLId(playerid)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Chi chu nha moi co the lam dieu nay.");
	...
	foreach(new p : Player) {
		...
		if(PlayerInfo[p][pVW] == PlayerInfo[playerid][pVW] && ProxDetectorS(50, playerid, p)) {
			...
			SendClientMessageEx(p, COLOR_GRAD1, "Ban se duoc chuyen den phien ban chua hoan thanh cua ngoi nha.");
...
CMD:furnishhouse(playerid, params[]) {
	...
	if(fHouseZ == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Nhiem vu nay khong the hoan thanh cho loai noi that ngoi nha cua ban.");
	if(HouseInfo[iHouseID][hInteriorZ] < fHouseZ + 25.0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ngoi nha cua ban da duoc trang bi.");
...
public OnEditFurniture() {
	...
	if(mysql_errno()) return SendClientMessageToAll(0, "[Do noi that] Khong luu.");
	return 1;
}
...
CMD:furnitureresetpos(playerid, params[]) {
	...
	else SendClientMessageEx(playerid, COLOR_GRAD1, "Ban chi co the su dung cai nay khi ban nga trong khi dinh vi mon do noi that.");
	return 1;
}
...
CMD:destroyfurniture(playerid, params[]) {
	...
	if(!IsValidFurniture(iHouseID, iSlotID, 1)) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da chi dinh mot vi tri khong hop le.");
	DestroyFurniture(iHouseID, iSlotID);
	return 1;
}
...
CMD:revokebuilders(playerid, params[]) {
	...
	switch(iHouseID) {
		case 1: {
			if(PlayerInfo[playerid][pPhousekey] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ngoi nha nay khong duoc su dung.");
			iHouseID = PlayerInfo[playerid][pPhousekey];
		}
		case 2: {
			if(PlayerInfo[playerid][pPhousekey2] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ngoi nha nay khong duoc su dung.");
			iHouseID = PlayerInfo[playerid][pPhousekey2];
		}
		case 3: {
			if(PlayerInfo[playerid][pPhousekey3] == INVALID_HOUSE_ID) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ngoi nha nay khong duoc su dung.");
			iHouseID = PlayerInfo[playerid][pPhousekey3];
		}
		default: return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban chi dinh ID nha khong hop le.");
	}
	...
	SendClientMessageEx(playerid, COLOR_YELLOW, "Tat ca cac quyen cua nha xay dung da bi thu hoi");
	return 1;
}
	...
	if(textm_Selected3DTextureMenu[playerid] != -1 || PlayerTextureMenuInfo[playerid][ptextm_TPreviewState] == PREVIEW_STATE_SELECT) {
		...
		if(PlayerTextureMenuInfo[playerid][ptextm_TPreviewState] == PREVIEW_STATE_SELECT) {
			...
			if(newkeys == KEY_NO) { // Next
				...
				PlayerTextDrawShow(playerid, Furniture_PTD[playerid][2]);
				format(szMiscArray, sizeof(szMiscArray), "Ten: %s", szTextureName[textm_SelectedTile[playerid]]);
				PlayerTextDrawSetString(playerid, Furniture_PTD[playerid][3], szMiscArray);
				PlayerTextDrawShow(playerid, Furniture_PTD[playerid][3]);
				return 1;
			}
			...
			if(newkeys == KEY_YES) { // Accept
				...
				SetPVarInt(playerid, "studorfind", 1);
				ShowPlayerDialogEx(playerid, DIALOG_FURNITURE_PAINT2, DIALOG_STYLE_LIST, "Menu Do noi that | Texturing", "Texture Studio\nSearch Texture\nColor Studio", "Lua chon", "Huy bo");
				return 1;
			}
