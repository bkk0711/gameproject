...
EntExit_GetID(playerid) {
	...
	for(new i; i < MAX_BUSINESSES; ++i) {
		if(IsPlayerInRangeOfPoint(playerid, 2.0, Businesses[i][bExtPos][0], Businesses[i][bExtPos][1], Businesses[i][bExtPos][2])) {
			if(PlayerInfo[playerid][pLevel] < 3 && Businesses[i][bType] == 13) return SendClientMessage(playerid, COLOR_LIGHTBLUE, "Dang buon, ban khong the vao Casino");
...
CMD:entersystem(playerid, params[]) {
	...
	if(iNewEnterSystem) {
		...
		iNewEnterSystem = 0;
		SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da tat he thong vao/ra.");
	}
	else {
		iNewEnterSystem = 1;
		SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da mo he thong vao/ra.");
	}
	return 1;
}
...
stock Vehicle_Enter(playerid, i) {
	...
	if(CarryCrate[playerid] != -1) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the dua thung vao loai interior nay!");
...
DDoor_Enter(playerid, i)
{
	...
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay truoc khi vao canh cua nay.", DoorTimer[playerid]-gettime());
	if(DDoorsInfo[i][ddVIP] > 0 && PlayerInfo[playerid][pDonateRank] < DDoorsInfo[i][ddVIP]) 
	{
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong du cap do VIP.");
		return 1;
	}
	
	if(DDoorsInfo[i][ddFamed] > 0 && PlayerInfo[playerid][pFamed] < DDoorsInfo[i][ddFamed]) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, ban khong du cap do famed.");
		return 1;
	}
	...
	if(DDoorsInfo[i][ddAllegiance] > 0) {
		if(!(0 <= PlayerInfo[playerid][pMember] < MAX_GROUPS) || arrGroupData[PlayerInfo[playerid][pMember]][g_iAllegiance] != DDoorsInfo[i][ddAllegiance]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Door nay la cua quoc gia.");
		else if(PlayerInfo[playerid][pRank] < DDoorsInfo[i][ddRank]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Cap bac cua ban khong du cao de vao day.");
	}
	...
	if(DDoorsInfo[i][ddGroupType] > 0) {
		if(!(0 <= PlayerInfo[playerid][pMember] < MAX_GROUPS) || arrGroupData[PlayerInfo[playerid][pMember]][g_iGroupType] != DDoorsInfo[i][ddGroupType] && arrGroupData[PlayerInfo[playerid][pMember]][g_iAllegiance] != DDoorsInfo[i][ddAllegiance]) {
			return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Door nay la cua to chuc.");
		}
		else if(PlayerInfo[playerid][pRank] < DDoorsInfo[i][ddRank]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Cap bac cua ban khong du cao de vao day.");
	}

	if(DDoorsInfo[i][ddFaction] != INVALID_GROUP_ID) {
		if(PlayerInfo[playerid][pMember] != DDoorsInfo[i][ddFaction]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Door nay la cua to chuc.");
		else if(PlayerInfo[playerid][pRank] < DDoorsInfo[i][ddRank]) return SendClientMessageEx(playerid, COLOR_GRAD2, "Cap bac cua ban khong du cao de vao day.");
	}

	if(DDoorsInfo[i][ddAdmin] > 0 && PlayerInfo[playerid][pAdmin] < DDoorsInfo[i][ddAdmin]) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong du cap do admin.");
		return 1;
	}

	if(DDoorsInfo[i][ddWanted] > 0 && PlayerInfo[playerid][pWantedLevel] != 0) {
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Door nay han che nhung nguoi co Wanted.");
		return 1;
	}

	if(DDoorsInfo[i][ddLocked] == 1) {
		return SendClientMessageEx(playerid, COLOR_GRAD2, "Door nay hien dang bi khoa.");
	}
...
{
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay ngay bay gio. Doi loading xong.");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay de roi khoi day.", DoorTimer[playerid]-gettime());
	if(DDoorsInfo[i][ddVIP] > 0 && PlayerInfo[playerid][pDonateRank] < DDoorsInfo[i][ddVIP]) 
	{
		SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the vao, Ban khong du cap do VIP.");
		return 1;
...
House_Enter(playerid, i) {
	...
	if(PlayerInfo[playerid][pPhousekey] == i || PlayerInfo[playerid][pPhousekey2] == i || HouseInfo[i][hLock] == 0 || PlayerInfo[playerid][pRenting] == i) {
		if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay truoc khi co the vao trong.", DoorTimer[playerid]-gettime());
		if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay ngay bay gio. Doi loading xong.");
		House_VistorCheck(i);
...
{
	if(Businesses[i][bExtPos][1] == 0.0) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the vao Business.");
	if(Businesses[i][bStatus]) {
		...
		if (Businesses[i][bType] == BUSINESS_TYPE_GYM)
		{
			...
			{
				GameTextForPlayer(playerid, "~r~Ban can tien de vao phong tap nay", 5000, 1);
				return 1;
			}
		}
		...
		{
			new string[50];
			format(string, sizeof(string), "Ban da vao mot phong tap va tinh phi $%i.", Businesses[i][bGymEntryFee]);
			...
			{
				SendClientMessageEx(playerid, COLOR_WHITE, "Go /beginswimming de bat dau su dung ho bio.");
				SendClientMessageEx(playerid, COLOR_WHITE, "Go /joinboxing de bat dau su dung san dau.");
			}
			else if (Businesses[i][bGymType] == 2)
			{
				SendClientMessageEx(playerid, COLOR_WHITE, "Go /beginparkour de bat dau parkour.");
			}
		}
		DoorTimer[playerid] = gettime()+2;
	}
...
Garage_Enter(playerid, i) {
	...
	if(GarageInfo[i][gar_Locked] == 1) return SendClientMessageEx(playerid, COLOR_GRAD2, "Nha de xe nay hien dang bi khoa.");
	if(GetPVarType(playerid, "StreamPrep")) return SendClientMessageEx(playerid, COLOR_WHITE, "Ban khong the lam dieu nay ngay bay gio. Doi loading xong.");
	if(gettime() < DoorTimer[playerid]) return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai doi %d giay truoc khi co the vao trong.", DoorTimer[playerid]-gettime());
	PlayerInfo[playerid][pVW] = GarageInfo[i][gar_InteriorVW];
