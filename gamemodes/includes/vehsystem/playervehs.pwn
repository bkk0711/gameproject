			...
			if(PlayerVehicleInfo[playerid][v][pvModelId] >= 400) {
				if(PlayerVehicleInfo[playerid][v][pvSpawned] && !PlayerVehicleInfo[playerid][v][pvDisabled] && !PlayerVehicleInfo[playerid][v][pvImpounded]) {
					...
					if(PlayerVehicleInfo[playerid][v][pvCrashFlag] == 1 && PlayerVehicleInfo[playerid][v][pvCrashX] != 0.0)
					{
						...
						SendClientMessageEx(playerid, COLOR_WHITE, "Xe cua ban da duoc khoi phuc den vi tri da biet cuoi cung cua chung tu thoi gian cho truoc do cua ban.");
					}
				}
...
stock UnloadPlayerVehicles(playerid, logoff = 0, reason = 0) {
	for(new v = 0; v < MAX_PLAYERVEHICLES; v++) if(PlayerVehicleInfo[playerid][v][pvId] != INVALID_PLAYER_VEHICLE_ID && !PlayerVehicleInfo[playerid][v][pvImpounded] && PlayerVehicleInfo[playerid][v][pvSpawned]) {
		...
		if(PlayerVehicleInfo[playerid][v][pvBeingPickLocked] > 0) {
			...
			format(szMessage, sizeof(szMessage), "Nguoi choi (%s) so huu chiec xe nay (%s) co %s.", GetPlayerNameEx(playerid), GetVehicleName(PlayerVehicleInfo[playerid][v][pvId]), rsMessage);
			SendClientMessageEx(extraid, COLOR_YELLOW, szMessage);
			new ip2[MAX_PLAYER_NAME];
			GetPlayerIp(extraid, ip2, sizeof(ip2));
			SendClientMessageEx(extraid, COLOR_YELLOW, "(( Xe spawn sau khi ban hoan thanh hoac khong giao hang. ))");
...
public ParkVehicle(playerid, ownerid, vehicleid, d, Float:X, Float:Y, Float:Z)
{
	if(IsPlayerInRangeOfPoint(playerid, 1.0, X, Y, Z))
	{
	    ...
	    GetVehicleHealth(vehicleid, health);
     	...
     	if(health < 800) return SendClientMessageEx(playerid, COLOR_GREY, " Xe cua ban bi hu qua khong the dau xe.");
		if(ownerid != INVALID_PLAYER_ID)
	    {
			...
			format(string, sizeof(string), "* %s da dau chiec xe %s's.", GetPlayerNameEx(playerid), GetPlayerNameEx(ownerid));
		}
		else
		{
		    GetVehiclePos(vehicleid, x, y, z);
			...
			PutPlayerInVehicle(playerid, vehicleid, 0);
			SetPlayerArmedWeapon(playerid, 0);
			format(string, sizeof(string), "* %s da dau chiec xe cua ho.", GetPlayerNameEx(playerid), GetPlayerNameEx(ownerid));
		}
		ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
	}
	else
	{
	    SendClientMessage(playerid, COLOR_WHITE, "Xe khong the dau vi ban da di chuyen!");
	}
	return 1;
}