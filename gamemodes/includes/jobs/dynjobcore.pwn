...
public UpdateJobPoint(id)
{
	...
	format(szMiscArray, sizeof szMiscArray, "{FFFF00}Diem cong viec ({FFFFFF}ID: %i{FFFF00})\n\nTen: {FFFFFF}%s\n{FFFF00}Nhan {FFFFFF}~k~~CONVERSATION_YES~ {FFFF00}de xin viec.", id, GetJobName(JobData[id][jType]));
...
Job_GetJob(playerid, i)
{
    ...
    {
        format(szMiscArray, sizeof(szMiscArray), "Ban can phai it nhat level %i de xin viec.", JobData[i][jLevel]);
    ...
    if(PlayerInfo[playerid][pJob] == JobData[i][jType] || PlayerInfo[playerid][pJob2] == JobData[i][jType] || PlayerInfo[playerid][pJob3] == JobData[i][jType]) {
    	return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da co cong viec nay.");
    }
	if(PlayerInfo[playerid][pJob] == 0) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc cong viec nhu mot {FFFF00}%s{FFFFFF}?", GetJobName(JobData[i][jType]));
	else if(0 < PlayerInfo[playerid][pDonateRank] < 4) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc cong viec nhu mot {FFFF00}%s{FFFFFF}? (VIP)", GetJobName(JobData[i][jType]));
	else if(PlayerInfo[playerid][pDonateRank] > 3) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc cong viec nhu mot {FFFF00}%s{FFFFFF}? (Platinum VIP)", GetJobName(JobData[i][jType]));
	else if(PlayerInfo[playerid][pFamed] > 0) format(szMiscArray, sizeof(szMiscArray), "Ban co muon tiep tuc cong viec nhu mot {FFFF00}%s{FFFFFF}? (OS/Famed)", GetJobName(JobData[i][jType]));
	else if(PlayerInfo[playerid][pJob] > 0 && PlayerInfo[playerid][pDonateRank] == 0 && PlayerInfo[playerid][pFamed] == 0) return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban dang co cong viec nay, dung '/quitjob' de nghi viec cu va bat dau lam viec moi.");
	
	SetPVarInt(playerid, PVAR_JOB_OBTAINING, JobData[i][jType]);
	Dialog_Show(playerid, acceptjob, DIALOG_STYLE_MSGBOX, "Diem Cong Viec", szMiscArray, "Dong Y", "Khong");
	return 1;
}
...
CMD:gotojob(playerid, params[]) {
	...
	if(PlayerInfo[playerid][pAdmin] >= 2)
	{
		...
		if(!(0 <= jobid < MAX_JOBPOINTS)) return SendClientMessageEx(playerid, COLOR_GRAD2, "ID cong viec phai lon hon 0 - 99.");
...
Dialog:acceptjob(playerid, response, listitem, inputtext[]) {
	...
	if(response) {
		...
		if(PlayerInfo[playerid][pJob] == 0)	{
			...
			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Chuc mung ban voi cong viec moi, go /help de xem lenh moi.");
			if(iJob == 21) {

				SendClientMessageEx(playerid, COLOR_WHITE, "Ban da duoc tang mot bo trang phuc Pizza Stack!");
				SendClientMessageEx(playerid, COLOR_WHITE, "Ban da duoc chap nhan tham gia Pizza Nation; mot trong nhung nhan vat nguy hiem nhat the gioi.");
            	SendClientMessageEx(playerid, COLOR_WHITE, "Ghi nho: Dung an Pizza than thanh, neu khong se phai doi mat voi mot cai chet dai, dau don sap xay ra.");
		...
		if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
            ...
            SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Chuc mung ban voi cong viec moi, go /help de xem lenh moi.");
            SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Day la mot cong viec thu hai cua ban.");
        ...
        if((PlayerInfo[playerid][pDonateRank] >= 3 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob3] == 0) {
		    ...
		    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Chuc mung ban voi cong viec moi, go /help de xem lenh moi.");
            SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Day la cong viec thu ba cua ban.");
		...
		return SendClientMessageEx(playerid, COLOR_GRAD1, "Ban khong the nhan them bat ky cong viec nao. Vui long dung /quitjob de nghi viec lam.");
    }
    return 1;
}
