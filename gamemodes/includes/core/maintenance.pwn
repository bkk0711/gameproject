...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	...
	switch(dialogid)
	{
		...
		{
			if(response) {
				if(isnull(inputtext)) return ShowPlayerDialogEx(playerid, DIALOG_MAINTENANCE, DIALOG_STYLE_INPUT, "Bo hen gio se chay trong bao lau?", "Vui long chi dinh trong vai giay truoc khi may chu khoi dong tat ca nguoi dung va tat?\n\nWARNING: Khong the hoan tac hanh dong nay!", "Tat", "Ra");
				if(!(30 <= strval(inputtext) < 3541)) return ShowPlayerDialogEx(playerid, DIALOG_MAINTENANCE, DIALOG_STYLE_INPUT, "Bo hen gio se chay trong bao lau?", "Vui long chi dinh trong vai giay truoc khi may chu khoi dong tat ca nguoi dung va tat?\n\nWARNING: Khong the hoan tac hanh dong nay!", "Tat", "Ra");
			    ...
			    SendClientMessageToAllEx(COLOR_LIGHTBLUE, "* May chu se ngung hoat dong va bao tri theo lich trinh. (Xem man hinh duoi cung ben phai)");
			    GameTextForAll("~n~~n~~n~~n~~y~] Canh Bao Bao Tri Da Lenh Lich ]", 5000, 3);
			...
			else SendClientMessageEx(playerid, COLOR_WHITE, "Ban da huy thuc hien khoi dong lai bao tri.");
...
CMD:announcem(playerid, params[])
{
    ...
    ShowPlayerDialogEx(playerid, DIALOG_MAINTENANCE, DIALOG_STYLE_INPUT, "Bo hen gio nen chay trong bao lau?", "Vui long chi dinh trong vai giay truoc khi may chu khoi dong tat ca nguoi dung va tat?\n\nWARNING: Khong the hoan tac hanh dong!", "Tat", "Ra");
    return 1;
}
...
{
	new string[128];
    ABroadCast(COLOR_YELLOW, "{AA3333}BAO TRI{FFFF00}: Dong Bang Tai Khoan...", 1);
		...
		foreach(new p : Player)
		{
			...
			if(arenaid == i)
			{
				if(PaintBallArena[arenaid][pbBidMoney] > 0)
				{
					...
					format(string,sizeof(string),"Ban da duoc hoan lai tong cong $%d vi dong cua som.",PaintBallArena[GetPVarInt(p, "IsInArena")][pbBidMoney]);
					...
					switch(PaintBallArena[arenaid][pbGameType])
					{
						case 1:
						{
							if(PlayerInfo[p][pDonateRank] < 3)
							{
								...
								format(string,sizeof(string),"Ban da duoc hoan lai tong cong %d Paintball Tokens vi dong cua som.",3);
								SendClientMessageEx(p, COLOR_WHITE, string);
							}
						}
						case 2:
						{
							if(PlayerInfo[p][pDonateRank] < 3)
							{
								...
								format(string,sizeof(string),"Ban da duoc hoan lai tong cong %d Paintball Tokens vi dong cua som.",4);
								SendClientMessageEx(p, COLOR_WHITE, string);
							}
						}
						case 3:
						{
							if(PlayerInfo[p][pDonateRank] < 3)
							{
								...
								format(string,sizeof(string),"Ban da duoc hoan lai tong cong %d Paintball Tokens vi dong cua som.",5);
								SendClientMessageEx(p, COLOR_WHITE, string);
							}
						}
						case 4:
						...
						case 5:
						{
							if(PlayerInfo[p][pDonateRank] < 3)
							{
								...
								format(string,sizeof(string),"Ban da duoc hoan lai tong cong %d Paintball Tokens vi dong cua som.",6);
								SendClientMessageEx(p, COLOR_WHITE, string);
							}
						}
					}
