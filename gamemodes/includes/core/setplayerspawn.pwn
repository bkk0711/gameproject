    ...
    if(IsPlayerConnected(playerid))
	{
		...
		if(HungerPlayerInfo[playerid][hgInEvent] == 1)
		{
			if(hgActive > 0)
			{
				...
				{
					new szmessage[128];
					format(szmessage, sizeof(szmessage), "** %s da dung thu 3 trong su kien Hunger Games.", GetPlayerNameEx(playerid));
					...
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "** Ban da duoc trao 10 co hoi rut tham cho su kien Fall Into Fun Event.");
				...
				{
					new szmessage[128];
					format(szmessage, sizeof(szmessage), "** %s da dung o vi tri thu 2 trong su kien Hunger Games.", GetPlayerNameEx(playerid));
					SendClientMessageToAll(COLOR_LIGHTBLUE, szmessage);
					...
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "**Ban da duoc trao 25 co hoi rut tham cho su kien Fall Into Fun Event.");
					...
					foreach(new i : Player) 
					{
						...
						{
							format(szmessage, sizeof(szmessage), "** %s da dung o vi tri thu nhat trong su kien Hunger Games.", GetPlayerNameEx(i));
							SendClientMessageToAll(COLOR_LIGHTBLUE, szmessage);
							...
							SendClientMessageEx(i, COLOR_LIGHTBLUE, "** Ban da duoc trao 5 co hoi rut tham cho su kien Fall Into Fun Event.");
							hgActive = 0;
				...
				else if(hgPlayerCount > 3 || hgPlayerCount == 1)
				{
					...
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da chet va bi xoa khoi su kien Hunger Games, chuc ban may man lan sau.");
		...
		if(PlayerInfo[playerid][pJailTime] > 0)
		{
		    ...
		    else if(strfind(PlayerInfo[playerid][pPrisonReason], "[OOC]", true) != -1)
		    {
				...
				new string[128];
				format(string, sizeof(string), "Ban dang o trong tu, ly do: %s", PlayerInfo[playerid][pPrisonReason]);
		...
		if(GetPVarInt(playerid, "Injured") == 1)
		{
		    switch(aLastShotWeapon[playerid])
			{
				...
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi tan cong boi vu khi %s.", Weapon_ReturnName(aLastShotWeapon[playerid]));
				}
				case WEAPON_KATANA, WEAPON_KNIFE:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi chem boi vu khi %s.", Weapon_ReturnName(aLastShotWeapon[playerid]));
				}
				case WEAPON_GRENADE, WEAPON_ROCKETLAUNCHER, WEAPON_HEATSEEKER, 51:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi thiet hai nghiem trong tu mot vu no.");
				}
				case WEAPON_FIREEXTINGUISHER, WEAPON_SPRAYCAN:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi dau doc va nghet tho.");
				}
				case WEAPON_MOLTOV, WEAPON_FLAMETHROWER:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi bong do lua.");
				}
				...
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi ban trong thuong trong %s boi mot %s.", ReturnBoneName(aLastShotBone[playerid]), Weapon_ReturnName(aLastShotWeapon[playerid]));
				}
				case WEAPON_VEHICLE, WEAPON_COLLISION:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi gay nhieu xuong.");
				}
				case WEAPON_DROWN:
				{
					format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bat tinh vi duoi nuoc.");
				}
				case 0: format(szMiscArray, sizeof(szMiscArray), "Nguoi choi bi danh dap da man boi tay khong.");
				default: format(szMiscArray, sizeof(szMiscArray), "Nguoi choi da qua doi tu nhung nguyen nhan khong xac dinh.");
			}
			strcat(szMiscArray, "\n{FF0000}(Nguoi choi bi thuong nang).", sizeof(szMiscArray));
		...
		if(GetPVarInt(playerid, "EventToken") == 1)
		{
            ...
            if(EventKernel[EventType] == 4)
			{
				...
				SendClientMessageEx(playerid, COLOR_WHITE, "Ban la mot Zombie! Su dung /bite de lay nhiem cho nguoi khac");
