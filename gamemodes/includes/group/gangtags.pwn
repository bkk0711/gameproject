	...
	switch(dialogid)
	{
		case DIALOG_GANGTAGS_INPUT:
		{
			...
			if(strlen(inputtext) > MAX_GANGTAGS_LEN) {
				...
				return SendClientMessage(playerid, COLOR_GRAD1, "Van ban cua ban qua dai.");
			}
			szMiscArray[0] = 0;

			format(szMiscArray, sizeof(szMiscArray), "{AA3333}AdmWarning{FFFF00}: %s (ID: %d) (T:%d) Phun %s", GetPlayerNameEx(playerid), playerid, GetPVarInt(playerid, PVAR_GANGTAGID), inputtext);
			...
			ShowPlayerDialogEx(playerid, DIALOG_GANGTAGS_FONT, DIALOG_STYLE_LIST, "Gang Tags | Font", szMiscArray, "Lua chon", "");
			return 1;
		}
		case DIALOG_GANGTAGS_FONT:
		{
			if(!response) return ClearAnimationsEx(playerid), TogglePlayerControllable(playerid, 1), 1;
			SendClientMessage(playerid, COLOR_GREEN, "[Gang Tags] {DDDDDD}Phun... Dung /tag de dung phun.");
...
public GangTag_InitSeconds(playerid, time, fontid)
{
	...
	format(szMiscArray, sizeof(szMiscArray), "~g~ Phun... ~w~ %d", timesec);
...
{
	if(mysql_errno()) print("[Gang Tags] Da xay ra loi khi chay query.");
...
public GangTag_OnLoad()
{
	...
	if(!iRows) return print("[Gang Tags] Khong co the gang trong co so du lieu.");
	new idx,
...
GangTag_Create(iPlayerID)
{
	...
	SendClientMessage(iPlayerID, COLOR_GRAD1, "Ban khong the tao bat ky tags gang nao.");
	return 1;
}
...
public GangTag_OnCreate(iPlayerID, i, Float:X, Float:Y, Float:Z)
{
	...
	if(!iRows)
	{
		...
		format(szMiscArray, sizeof(szMiscArray), "Ban da tao thanh cong mot diem tag gang (ID %d)", i);
...
{
	if(mysql_errno()) return SendClientMessage(iPlayerID, COLOR_GRAD1, "Da xay ra loi. Thu lai sau.");
	format(szMiscArray, sizeof(szMiscArray), "Ban da chinh sua thanh cong tag Gang ID %d", i);
...
public GangTag_OnDelete(iPlayerID, i)
{
	...
	format(szMiscArray, sizeof(szMiscArray), "Ban da xoa thanh cong gang tag ID: %d", i);
	SendClientMessage(iPlayerID, COLOR_YELLOW, szMiscArray);
	return 1;
}
...
public OnGetGangTags(iPlayerID)
{
	...
	if(iRows > 0)
	{
		...
		ShowPlayerDialogEx(iPlayerID, DIALOG_GANGTAGS_LIST, DIALOG_STYLE_TABLIST_HEADERS, "Gang Tags | Danh sach", szMiscArray, "Lua chon", "");
		return 1;
	}
	return SendClientMessage(iPlayerID, COLOR_GRAD1, "Khong co gangs tags trong co so du lieu.");
}

CMD:gangtaghelp(playerid, params[])
{
	SendClientMessage(playerid, COLOR_GREEN, "______________________________");
	SendClientMessage(playerid, COLOR_GRAD1, "Gang Tags | Lenh");
	...
	if(IsAStreetSweeper(playerid) || strfind(PlayerInfo[playerid][pPrisonReason], "[IC]", true) != -1) {
		...
		for(new i; i < MAX_GANGTAGS; ++i)
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 5.0, gtPos[0], gtPos[1], gtPos[2]))
			{
				...
				GameTextForPlayer(playerid, "~y~Xoa...", 5000, 3);
				SetTimerEx("GangTag_OnCleanTag", 15000, false, "ii", playerid, i);
				return 1;
			}
		}
		return SendClientMessage(playerid, COLOR_GRAD1, "Ban can phai o gan mot buc tuong de phun de lam sach no.");
	}
	else SendClientMessage(playerid, COLOR_GRAD1, "Ban phai la nguoi quet duong hoac cong ty ve sinh khac de su dung lenh nay.");
	return 1;
}
...
CMD:tag(playerid, params[])
{
	if(GetPVarType(playerid, PVAR_GANGTAGTEXT))
	{
		...
		ClearAnimationsEx(playerid);
		TogglePlayerControllable(playerid, 1);
		GameTextForPlayer(playerid, "~r~Huy bo!", 5000, 3);
		return 1;
	}
	if(!IsACriminal(playerid)) return SendClientMessage(playerid, COLOR_GRAD1, "Ban phai o trong Gang de su dung lenh nay.");
	if(PlayerInfo[playerid][pRank] == 0) return SendClientMessage(playerid, COLOR_GRAD1, "Ban can phai co it nhat rank 1 de tag.");
	if(GetPlayerWeapon(playerid) != 41) return SendClientMessage(playerid, COLOR_GRAD1, "Ban can mot binh xit de tag tren tuong.");
	new Float:gtPos[3];
	for(new i; i < MAX_GANGTAGS; ++i)
	{
		GetDynamicObjectPos(arrGangTags[i][gt_iObjectID], gtPos[0], gtPos[1], gtPos[2]);
		if(IsPlayerInRangeOfPoint(playerid, 5.0, gtPos[0], gtPos[1], gtPos[2]))
		{
			...
			TogglePlayerControllable(playerid, 0);
			ApplyAnimation(playerid, "SPRAYCAN", "spraycan_fire", 4.1, 1, 1, 1, 1, 0, 0);
			ShowPlayerDialogEx(playerid, DIALOG_GANGTAGS_INPUT, DIALOG_STYLE_INPUT, "Gang Tag Point | Chen van ban", "Chen van ban ban muon phun len tuong", "Phun", "");
			return 1;
		}
	}
	SendClientMessage(playerid, COLOR_GRAD1, "Ban khong o gan diem tag.");
	return 1;
}
